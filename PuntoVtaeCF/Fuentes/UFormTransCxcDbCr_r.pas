...
uses
  ...
  ShellApi, ComCtrls, IBTable, RXSpin;
...
type
  TfrmTransCxcProc_R = class(TForm)
    ...
    Label12: TLabel;
    lblNumTranscxc: TLabel;
    Label13: TLabel;
    Label14: TLabel;
    lblLastNCR: TLabel;
    lblLastNDB: TLabel;
    ibsqlLastNumDoc: TIBSQL;
    ibsqlUltNumTrnxcx: TIBSQL;
    rxspinImpCantCopias: TRxSpinEdit;
    Label45: TLabel;
    qrytrncxcbydoc: TIBQuery;
    qrytrncxcbydocSERIE: TIntegerField;
  ...
  private
    ...
    procedure ProcUpdateFactPendienteNCR(numero_fact:integer;
              monto_abonado:currency;codigo_cte_in:integer;seriecxc:integer);
    ...
    function buscarUltNumTrnxcx:string;
    procedure setUltimoNCRNDB;
...
uses UDatModConectar, UGlobal, UDatModCxc, UDatModIngresos,
  ...
  UDatModFactura, UFormBuscarNCF, UTestImpFiscal, UDatModReportes,
  uFormAsignareCFVenta;
...
begin
  ...
  lblNumTranscxc.Caption:= buscarUltNumTrnxcx;//funcion
  setUltimoNCRNDB;
end;
...
begin
  ...
  try
    frmAsignareCFVenta.IBDataSet1.Close;
      ...
      frmAsignareCFVenta.BitBtn13Click(Self); //GENERAR NCF
      ...
      frmAsignareCFVenta.IBDataSet1MONTO.Value      := Monto;
  ...
  try
    frmAsignarNCFVenta.IBDataSet1.Close;
      ...
      frmAsignarNCFVenta.BitBtn13Click(Self); //GENERAR NCF
...
begin
  ...
  try
    ...
    begin
      GlbTFiscalNCR:= dmFactura.ibQryViewNCFTIPO_CF.Value;
  ...
  dmcxc.qryRepTransCxcNDB_NCR.RecordCount;

  //Nota de credito fiscal pediente de programacion
  //¿Como se hace una NCR para solo monto?
  if (GlbActivaIFiscal = -1) and (GlbModeloImpresora <> GLBIFISCALOKI407FP) then
  begin
    ...
    try
      ...
      IntToStr(VarUsuarioGlb),StrLeftPad(IntToStr(dmcxc.transCxcNotaCreditoNUM_DOC.Value),8),'.txt']);
      PrepararInfoFiscal(dmcxc.qryRepTransCxcNDB_NCRSERIE.AsInteger);
      frmTestImpresoraFiscal.ncfAfectado := Edit1.Text;
      ...
      //frmTestImpresoraFiscal.BitBtn1Click(Self);//crear archivo txt
      //archivo:= GlbExeIFiscal;
      //parameters:=Format('/PortComm=%s /Baudrate=%s /TipoDoc=DVF /Status=not /ImprimeZ=0 /CierreX=False /FileToPrint=%s', [IntToStr(GlbPuerto),IntToStr(GlbBaudRate),GlbNombreFiletoPrint]);
...
begin
  ReNumDocumento:=-1;
  qCodCte.Close;
  qCodCte.Params[0].Value := glbCia_Key;   
  qCodCte.Open;
  rxAcumulador.Close;
  rxAcumulador.Open;
  cont:=0;
...
begin
  ...
  begin
    ...
    if (ncfAfectado <> '') then
    dmcxc.tblTransCxcNUM_NCF_AFECTADO.Value:= ncfAfectado
    else if (edit2.text <> '') then
    dmcxc.tblTransCxcNUM_NCF_AFECTADO.Value:= edit2.text
    else if (edit1.text <> '') then
    dmcxc.tblTransCxcNUM_NCF_AFECTADO.Value:= edit1.text;
    ...
    dmcxc.transCxcNotaCredito.Append;
    ...
    GlbSalvarQuery(dmcxc.transCxcNotaCredito);
...
begin
  ...
  if (ReNumDocumento > 0) then
  begin
    qrytrncxcbydoc.Close;
    qrytrncxcbydoc.Params[0].Value:= ReNumDocumento;
    qrytrncxcbydoc.Open;
    qrytrncxcbydoc.First;

    dmcxc.qryRepTransCxcNDB_NCR.Close;
    dmcxc.qryRepTransCxcNDB_NCR.Params[0].Value:= qrytrncxcbydocSERIE.AsInteger;// ReNumDocumento;
    dmcxc.qryRepTransCxcNDB_NCR.Params[1].Value:= 5;//Nota Credito
    dmcxc.qryRepTransCxcNDB_NCR.Open;
    dmcxc.qryRepTransCxcNDB_NCR.First;
    if dmcxc.qryRepTransCxcNDB_NCR.RecordCount = 0 then
    begin
      MessageDlg('Número doc no encontrado, verifique.',mtInformation,[mbok],0);
      Exit;
    end;    
  //ignorado hasta que OKI Support me envie la forma correcta de crear el txt para el spooler.
  //temporal if (GlbActivaIFiscal = 1) then
  if (GlbActivaIFiscal = 1) and (GlbModeloImpresora <> GLBIFISCALOKI407FP) then
  begin
    frmTestImpresoraFiscal:=TfrmTestImpresoraFiscal.Create(Nil);
    try
      ...
      GlbNombreFiletoPrint:= Format('%s%s%s%s%s%s%s',[dmReportes.qryDatosCajaImpUserRUTA_TICKET.Value,'\ReImpNotaCredito',
                             Trim(GlbCodSucursal),Trim(GlbModeloImpresora),
                             IntToStr(VarUsuarioGlb),StrLeftPad(IntToStr(dmcxc.qryRepTransCxcNDB_NCRNUM_DOC.Value),8),'.txt']);

      frmTestImpresoraFiscal.btnreimprimeNotadeCreditoClick(Self);//crear archivo txt
...
begin
  ...
  begin
    ...
    dmCxc.tblTransCxcTIPO_SERV.value:=1;
    dmCxc.tblTransCxcMONEDA.Value   :='1';
    dmCxc.tblTransCxcVALOR_DOCUMENTO.Value:= qryFactPendienteMONTO_PENDIENTE.Value;
    ...
    dmcxc.tblTransCxcTIPO_DOC.Value:=5;
    if frmTransCxcProc_R.Showing then
...
begin
  ...
  begin
    ...
    if (xNumDoc < 0 ) then
    begin
      stpProcNumDoc.Params[1].Value:= dmcxc.tblTransCxcTIPO_DOC.Value;
      stpProcNumDoc.ExecProc;
      xNumDoc := stpProcNumDoc.Params[0].Value;
    end;
    ...
    dmcxc.transCxcNotaCreditoSERIE.Value := dmcxc.tblTransCxcserie.AsInteger;
    //dmcxc.transCxcNotaCredito ncfAfectado
    dmcxc.transCxcNotaCreditoNUM_DOC.Value := xNumDoc;
    dmcxc.transCxcNotaCreditoNUMERO_FACT.Value:= dmCxc.tblTransCxcNUMERO_DOC.Value;
    ...
    //numero_fact:integer;monto_abonado:currency;codigo_cte_in:integer;seriecxc:integer);
    ProcUpdateFactPendienteNCR(dmCxc.tblTransCxcNUMERO_DOC.Value,
    dmCxc.tblTransCxcVALOR_DOCUMENTO.Value,
    dmCxc.tblTransCxcCODIGO_CLIENTE.Value,
    dmCxc.tblTransCxcSerie.AsInteger);

    //Habilitar para Reporte
    dmcxc.qryRepTransCxcNDB_NCR.Close;
    dmcxc.qryRepTransCxcNDB_NCR.Params[0].Value:= dmcxc.transCxcNotaCreditoSERIE.Value;
    dmcxc.qryRepTransCxcNDB_NCR.Params[1].Value:= dmcxc.tblTransCxcTIPO_DOC.Value; 
    dmcxc.qryRepTransCxcNDB_NCR.Open;//dmcxc.qryRepTransCxcNDB_NCR.SQL.TEXT
...
begin
  ...
  if (Edit2.Text = '') then
  begin
    if not dmcxc.tblTransCxcNUM_NCF_AFECTADO.isNull then
    Edit2.Text:= dmcxc.tblTransCxcNUM_NCF_AFECTADO.Value;
  end;
end;
...
begin
  ...
  begin
    ...
    dmcxc.qryRepTransCxcNDB_NCR.Close;
    dmcxc.qryRepTransCxcNDB_NCR.Params[0].Value:= qrytrncxcbydocSERIE.AsInteger;//ReNumDocumento;
    dmcxc.qryRepTransCxcNDB_NCR.Params[1].Value:= 3;//3;//Nota Credito
    //dmcxc.qryRepTransCxcNDB_NCR.Params[2].Value:= dmcxc.transCxcNotaCreditoSERIE.Value;
    dmcxc.qryRepTransCxcNDB_NCR.Open;
    dmcxc.qryRepTransCxcNDB_NCR.First;
    if dmcxc.qryRepTransCxcNDB_NCR.RecordCount = 0 then
    begin
...
procedure TfrmTransCxcProc_R.SpeedButton1Click(Sender: TObject);
begin
   frmBuscarNCF:=TfrmBuscarNCF.Create(Nil);
  //try
   frmBuscarNCF.qryDatosNCF.Close;
   frmBuscarNCF.qryDatosNCF.Params[0].Value:= qCodCteCODIGO_CTE.Value;
   frmBuscarNCF.qryDatosNCF.Open;
   if frmBuscarNCF.qryDatosNCF.Locate('NUMERO_FACTURA', qryFactPendienteNUMERO_FACT.Value,[]) then
   begin
     edit2.Text  := frmBuscarNCF.qryDatosNCFNUMERO_NCF.Value;
     Edit1.Text  := frmBuscarNCF.qryDatosNCFNUMERO_NCF.Value;
     ncfAfectado := frmBuscarNCF.qryDatosNCFNUMERO_NCF.Value;
   end;
   if frmBuscarNCF.Showmodal = mrOk then
   begin
     edit2.Text  := frmBuscarNCF.qryDatosNCFNUMERO_NCF.Value;
     Edit1.Text  := frmBuscarNCF.qryDatosNCFNUMERO_NCF.Value;   
     ncfAfectado   := frmBuscarNCF.qryDatosNCFNUMERO_NCF.Value;
   ...
   GlbTFiscalNCR := frmBuscarNCF.qryDatosNCFTIPO_CF.Value;

   NoScroll := False;
   qryFactPendiente.Locate('NUMERO_FACT', frmBuscarNCF.qryDatosNCFNUMERO_FACTURA.Value,[]);
   Edit2.Text := ncfAfectado;
   Edit1.Text := ncfAfectado;
   NoScroll   := True;
  //finally
  //frmBuscarNCF.Free;
  //frmBuscarNCF:=Nil;
...
function TfrmTransCxcProc_R.buscarUltNumTrnxcx: string;
begin
  ibsqlUltNumTrnxcx.Close;
  ibsqlUltNumTrnxcx.ExecQuery;
  Result := ibsqlUltNumTrnxcx.FieldByName('numeromax').AsSTring;
end;

procedure TfrmTransCxcProc_R.setUltimoNCRNDB;
begin
  ibsqlLastNumDoc.Close;
  ibsqlLastNumDoc.ExecQuery;
  while not ibsqlLastNumDoc.Eof do
  begin
    if (ibsqlLastNumDoc.FieldByName('tipo_doc').asInteger = 3) then
    lblLastNdb .Caption:=ibsqlLastNumDoc.FieldByName('num_doc').AsSTring
    else
    lblLastNCR.Caption:=ibsqlLastNumDoc.FieldByName('num_doc').AsSTring;
    ibsqlLastNumDoc.next;
  end;
end;

procedure TfrmTransCxcProc_R.PrepararInfoFiscal(numerotrn: integer);
begin
  ...
  dmreportes.qryDatosReimpDocFiscal.Params[0].Value:= dmcxc.qryRepTransCxcNDB_NCRNUMERO_TRN_ORIGEN.Value;
  dmreportes.qryDatosReimpDocFiscal.open;
  dmreportes.qryDatosReimpDocFiscal.RecordCount;
...
procedure TfrmTransCxcProc_R.ProcUpdateFactPendienteNCR(numero_fact:integer;
monto_abonado:currency;codigo_cte_in:integer;seriecxc:integer);
begin
  //stp_UpdateCxc.Params[0].Value:= //Valor_Pendiente;
  stp_UpdateCxc.Params[1].Value:= 2;//tipo
  stp_UpdateCxc.Params[2].Value:= '1';//moneda
  stp_UpdateCxc.Params[3].Value:= numero_fact;
  stp_UpdateCxc.Params[4].Value:= monto_abonado;
  stp_UpdateCxc.Params[5].Value:= 'A';//status
  stp_UpdateCxc.Params[6].Value:= 1;//accion 1 = abono
  stp_UpdateCxc.Params[7].Value:= CODIGO_CTE_IN;
  stp_UpdateCxc.Params[8].Value:= GlbFechaTrnDiaria;
  stp_UpdateCxc.Params[9].Value:= seriecxc;
