...
begin
  ...
  if (edtcodProv.Text = '') or (CheckBox1.Checked) then
  dmcxp.qryFactPendCxp.SQL.Text:=dmcxp.qryFactPendCxp_Base.SQL.text
  else
  begin
    dmcxp.qryFactPendCxp.SQL.Text:=dmcxp.qryFactPendCxp_Base.SQL.text +
...
begin
  ...
  DateTimePicker1.Date:= GlbPrimerDiaMes(GlbFechaTrnDiaria);
  DateTimePicker2.Date:= GlbUltimoDiaMes(GlbFechaTrnDiaria);
end;
