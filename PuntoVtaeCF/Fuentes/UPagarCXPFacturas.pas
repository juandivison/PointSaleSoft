...
type
  TfrmPagarCXPFacturas = class(TForm)
    ...
    Label3: TLabel;
    qryFactPendienteFECHA_VENCE: TDateTimeField;
    Label4: TLabel;
...
uses UDatModConectar, UGlobal, UDatModCxc, UDatModIngresos,
  ...
  URegNumChequeCxp, URegNumTarjetaPagoCxp, Math;
...
begin
  ...
  valor1:=SumarFactAcobrar;
  _dif:=valor1 - edtMontoAcobrar.ValueFloat;
  if ABS(_dif) > 0.10 then
  begin
    MessageDlg('Favor verificar monto factura(s) a pagar. Dif'+InsertarComa(FloatToStr(_dif)), mtError, [mbOK], 0);
    Exit;
  end;
...
begin
  ...
  begin
    ...
    if ABS(rxAcumuladorMonto.Value) <= 0.01 then
    rxAcumuladorMonto.Value:=0;
...
procedure TfrmPagarCXPFacturas.RxDBGrid1GetCellParams(Sender: TObject;
  Field: TField; AFont: TFont; var Background: TColor; Highlight: Boolean);
begin
  if (qryFactPendienteFECHA_VENCE.Value > GlbFechaTrnDiaria) then
  begin
    BackGround:= clYellow;
    AFont.Color:= clBlack;
  end;
...
procedure TfrmPagarCXPFacturas.qryFactPendienteAfterOpen(
  DataSet: TDataSet);
var
  monto : Extended;
begin
