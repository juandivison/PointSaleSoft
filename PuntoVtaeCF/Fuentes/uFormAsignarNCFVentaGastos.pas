unit uFormAsignarNCFVentaGastos;
...
type
  TfrmAsignarNCFGastos = class(TForm)
    IBDataSet1: TIBDataSet;
    IBDataSet1SERIE: TIntegerField;
    IBDataSet1TIPO_NCF: TIBStringField;
    IBDataSet1NUMERO_NCF: TIBStringField;
...
var
  frmAsignarNCFGastos: TfrmAsignarNCFGastos;

implementation
uses UDatModConectar, UGlobal, UDatModFactura, UFormSelFecha,
  USeleccionarNCF, UFormHistSecNCF, USeleccionarNCFGastos;

{$R *.dfm}


procedure TfrmAsignarNCFGastos.BitBtn10Click(Sender: TObject);
begin
  IBDataSet1.prior;
end;

procedure TfrmAsignarNCFGastos.BitBtn11Click(Sender: TObject);
begin
  IBDataSet1.next;
end;

procedure TfrmAsignarNCFGastos.BitBtn12Click(Sender: TObject);
begin
  IBDataSet1.last;
end;

procedure TfrmAsignarNCFGastos.BitBtn1Click(Sender: TObject);
begin
...
procedure TfrmAsignarNCFGastos.BitBtn2Click(Sender: TObject);
begin
  if IBDataSet1.State in [dsBrowse] then
  IBDataSet1.Edit;
end;

procedure TfrmAsignarNCFGastos.BitBtn3Click(Sender: TObject);
begin
...
procedure TfrmAsignarNCFGastos.BitBtn4Click(Sender: TObject);
begin
  if IBDataSet1.State in [dsInsert, dsEdit] then
  begin
...
procedure TfrmAsignarNCFGastos.BitBtn5Click(Sender: TObject);
begin
...
procedure TfrmAsignarNCFGastos.BitBtn9Click(Sender: TObject);
begin
  IBDataSet1.First;
end;

procedure TfrmAsignarNCFGastos.BitBtn7Click(Sender: TObject);
begin
...
procedure TfrmAsignarNCFGastos.FormCreate(Sender: TObject);
begin
  dmFactura.tblTipoCF.Close;
  dmFactura.tblTipoCF.Open;
end;

function TfrmAsignarNCFGastos.ProcesaNCF(NCF: string): Boolean;
var
  strX : String[8];
  xSalir : Boolean;
begin
  xSalir:=False;
    ...
    if xSalir then
    begin
      if not ibStpMaxNCF.Params[06].IsNull then
      MessageDlg('Debe revisar la configuración de los NCF.',mtError, [mbOK], 0)
      else
...
var
  ...
  xSalir : Boolean;
begin
    frmSelNCFGastos:=TfrmSelNCFGastos.Create(nil);
      try
        dmfactura.ibqryViewNCFGastos.Filtered := False;

        if dmfactura.ibqryViewNCFGastos.Locate('TIPO_CF',NCF,[]) then
        begin
              descNCF:= dmfactura.ibQryViewNCFGastosDESCRIPCION.Value;
              ibStpMaxNCF.Params[7].Value := dmfactura.ibQryViewNCFGastosSERIE.Value;
              ibStpMaxNCF.Params[8].Value := dmfactura.ibQryViewNCFGastosDIV_NEGOCIO.Value;
              ibStpMaxNCF.Params[9].Value := dmfactura.ibQryViewNCFGastosPECF.Value;
              ibStpMaxNCF.Params[10].Value := dmfactura.ibQryViewNCFGastosAICF.Value;
              ibStpMaxNCF.Params[11].Value :=dmfactura.ibQryViewNCFGastosTIPO_CF.Value;
              ibStpMaxNCF.ExecProc;
    ...
    frmSelNCFGastos.free;
    frmSelNCFGastos:=nil;
    end;
    if xSalir then
    begin
      MessageDlg(ibStpMaxNCF.Params[06].Value+' o se generó algun otro error, verifique.',mtError, [mbOK], 0);
      Result:=False;
    end else Result:=True;
end;}

procedure TfrmAsignarNCFGastos.BitBtn13Click(Sender: TObject);
begin
  if not IBDataSet1TIPO_NCF.IsNull then
  begin
    if not ProcesaNCF(IBDataSet1TIPO_NCF.Value) then
    MessageDlg('Error generando NCF de gastos, verifique',mtInformation, [mbOK], 0)
    else IBDataSet1NUMERO_NCF.Value:=ncfGenerado;
  end;
end;

procedure TfrmAsignarNCFGastos.HistoricoNCF1Click(Sender: TObject);
begin
