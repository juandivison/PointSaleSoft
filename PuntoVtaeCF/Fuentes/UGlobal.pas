...
interface
 ...
 var
  ...
  nombrearchivo:string);
   ...
   procedure ProcNCFAfectado(serie:Integer);
   
implementation

uses UDatModConectar, UDatosVentas, UDatModCompania, UDatModControl,UcheckSecNCF,DelphiZXingQRCode,
...
begin
  ...
  begin
    dmreportes.qryNCFAfectado.SQL.Text:=dmReportes.qryNCFAfectado_0.sql.text;
    dmreportes.qryNCFAfectado.Params[0].Value:= serie;
  end else
  begin
    dmreportes.qryNCFAfectado.Close;
    dmreportes.qryNCFAfectado.SQL.Text:=dmReportes.qryNCFAfectado_1.sql.text;
    dmreportes.qryNCFAfectado.Params[0].Value:= serie;
  end;
...
begin
  ...
  F.Position := F.Size;
  s := FormatDatetime('yyyy-mm-dd hh:mm:ss',now)+LF+'PC: '+LF+CR+strUserName
  + LF+GlbUsuarioLogueado+LF
  + LF+ 'Codigo User:' +IntToStr(VarUsuarioGlb)+ LF+CR
  + linea +LF;
