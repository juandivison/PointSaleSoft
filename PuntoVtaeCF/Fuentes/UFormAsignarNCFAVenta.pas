...
begin
  ...
  begin
    ...
    dmFactura.ibQryViewNCF.Locate('TIPO_CF', '31', []);
  end else
  ...
  begin
    ...
    begin
      ...
      if GlbActivaECF = 1 then
      begin                               
      frmAsignareCFVenta := TfrmAsignareCFVenta.Create(Self);
      try
        frmAsignareCFVenta.IBDataSet1.Close;
        frmAsignareCFVenta.IBDataSet1.Open;
        frmAsignareCFVenta.IBDataSet1.Append;    
        ...
        if GlbcodVendedor > 0 then
          frmAsignareCFVenta.IBDataSet1CODIGO_USUARIO.Value := GlbcodVendedor
        else
          frmAsignareCFVenta.IBDataSet1CODIGO_USUARIO.Value := VarUsuarioGlb;
        frmAsignareCFVenta.IBDataSet1STATUS.Value := 'A';

        frmAsignareCFVenta.IBDataSet1TIPO_NCF.Value := dmFactura.ibQryViewNCFTIPO_CF.Value;
        frmAsignareCFVenta.BitBtn13Click(Self);

        frmAsignareCFVenta.Asginarncf := frmAsignareCFVenta.ncfGenerado;

        frmAsignareCFVenta.IBDataSet1NUMERO_eCF.Value := frmAsignareCFVenta.ncfGenerado;
        frmAsignareCFVenta.IBDataSet1MONTO.Value := _totalnetoValue;

        frmAsignareCFVenta.BitBtn4Click(Self);
        ...
        begin
          ...
          dmfactura.tblUpdateVentaSERIE_NCF_ASIGNADO.Value := frmAsignareCFVenta.IBDataSet1SERIE.Value;
