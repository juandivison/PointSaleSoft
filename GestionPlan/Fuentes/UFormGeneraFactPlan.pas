...
type
  TfrmGeneraFactPlan = class(TForm)
    ...
    RxLabel1: TRxLabel;
    chboxSinNCF: TCheckBox;
    ImprimirRecibo1: TCheckBox;
...
uses UDatModFactura, uglobal, UDatModCxc,
  ...
  UFactServicio, UDatmodDatosGenerales, URepFacturaPuntoVenta;
{$R *.dfm}
...
begin
  ...
  begin
    ...
    dmfactura.qryFactReimpMaster.Params[0].Value:= fini;
    dmfactura.qryFactReimpMaster.Params[1].Value:= ffin;
    dmfactura.qryFactReimpMaster.Open;

    dmFactura.qryFactServFReimpMaster.Next;
    if dmfactura.qryFactReimpDet.state = dsbrowse then
    begin
      dmfactura.qryFactReimpDet.Last;
      tRecordsDet := dmfactura.qryFactReimpDet.RecordCount;
    end else tRecordsDet:=1;
    dmFactura.qryFactServFReimpMaster.First;
...
begin
  ...
  Begin
    ...
    if dmDatos.qryPrecioTipoPlan.RecordCount = 0 then
    begin
    ...
    if dmFactura.ibQryViewNCF.locate('TIPO_CF', dmDatos.tblPlanFunerarioTIPO_NCF.Value,[]) and
    chboxSinNCF.Checked then
    begin
...
begin
  ...
  ibStpInsertVentaDet.Params[1].Value  := dmDatos.tblPlanFunerarioCOD_SERVICIO.Value;//rxVentaCodArticulo.Value;
...
begin
  ...
  dmCompania.tblCompania.Open;
  dmfactura.qryFactReimpMaster.Close;
  dmfactura.qryFactReimpMaster.Params[0].Value:= f;
  dmfactura.qryFactReimpMaster.Params[1].Value:= f;  
...
procedure TfrmGeneraFactPlan.FormShow(Sender: TObject);
begin
