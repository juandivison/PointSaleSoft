...
type
  TQckRepCksNomPer = class(TQuickRep)
    QRBand1: TQRBand;
    QRBand3: TQRBand;
    QRDBText3: TQRDBText;
...
var
  QckRepCksNomPer: TQckRepCksNomPer;
  cont : integer = 0;
implementation
...
procedure TQckRepCksNomPer.qrLabelMontoLetrasPrint(sender: TObject;
  var Value: String);
begin
  Value :='***'+UpperCase(Value)+'***';
end;

procedure TQckRepCksNomPer.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TQckRepCksNomPer.QckRepCksNomPerBeforePrint(
  Sender: TCustomQuickRep; var PrintReport: Boolean);
begin
  cont := 0;
end;

procedure TQckRepCksNomPer.QckRepCksNomPerAfterPrint(Sender: TObject);
begin
  cont := 0;
//  if not  dmcon.IBDatabase1.Transaction.InTransaction then
//  dmcon.IBDatabase1.Transaction.StartTransaction;
//  try
//    dmcon.IBDatabase1.Transaction.CommitRetaining;
//  except
//  dmcon.IBDatabase1.Transaction.RollbackRetaining;
//  end;
end;

procedure TQckRepCksNomPer.QckRepCksNomPerAfterPreview(Sender: TObject);
begin
  cont := 0;
  if not dmNomina.StpNumCksNom.Transaction.InTransaction then
  dmNomina.StpNumCksNom.Transaction.StartTransaction;
  try
    dmNomina.StpNumCksNom.Transaction.CommitRetaining;
  except
  dmNomina.StpNumCksNom.Transaction.RollbackRetaining;
  end;
end;
