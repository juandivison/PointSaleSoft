...
begin
  ...
  if (dmFactura.qryVentaFacturaMONTOPREPAGADO.IsNull) or (dmFactura.qryVentaFacturaMONTOPREPAGADO.Value =0) then
  begin
    PageFooterBand1.Height:=86;
  end;
...
procedure TqckRepReciboReimpTicketSurtidoraNew.ChildBand4BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  if FileExists('qrEncuesta.jpg') then
...
procedure TqckRepReciboReimpTicketSurtidoraNew.ChildBand13BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  if qckRepReciboReimpTicketSurtidora = nil then exit;
  if (GlbImpQREncuesta = 0) then
  begin
     ChildBand13.HasChild:=False;
     qckRepReciboReimpTicketSurtidora.Page.Length:=qckRepReciboReimpTicketSurtidora.Page.Length - 1.25;
  end else
  begin
    if not FileExists('qrEncuesta.jpg') then
    begin
      ChildBand13.HasChild:=False;
      qckRepReciboReimpTicketSurtidora.Page.Length:=qckRepReciboReimpTicketSurtidora.Page.Length - 1.25;
    end;
