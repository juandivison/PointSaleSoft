...
type
  TFrmBuscarClientesPersonas = class(TForm)
    ...
    qryPrestamosCOMENTARIO: TMemoField;
    qryPrestamosCONDICION: TIntegerField;
    qryPrestamosPRECIO_ID: TIntegerField;
    qryPrestamosTIPO_CF: TIBStringField;
    qryPrestamosMOVIL1: TIBStringField;
    qryPrestamosMOVIL2: TIBStringField;
    qryPrestamosCODIGO_TEXTO: TIBStringField;
    qryPrestamosUSARLEVELPRECIO: TSmallintField;
    ...
    qryClienteStdTIPO_CF: TIBStringField;
    qryClienteStdMOVIL1: TIBStringField;
    qryClienteStdMOVIL2: TIBStringField;
    qryClienteStdCODIGO_TEXTO: TIBStringField;
    qryClienteStdUSARLEVELPRECIO: TSmallintField;
...
//var
  //dsAnterio:String;
begin
   if RadioGroup1.ItemIndex = 5 then exit;//esto hasta que agregues apodo a la tabla cliente
  ...
  begin
    ...
    6: GlbOrderBy :=' order by NOMBRE_ABREV';
  ...
  begin
    if Not qryPrestamos.Locate('RNC_NUMERO',Edit1.Text,[loCaseInsensitive,loPartialKey]) then
  ...
  begin
    if not qryPrestamos.Locate('NOMBRE_ABREV',Edit1.Text,[loCaseInsensitive,loPartialKey]) then
    dbgrid1.Visible:=False else  dbGrid1.Visible:=True;
  end;
  Edit1Enter(Self);
  ...
  begin
    ...
    xCedula:=qryPrestamosRNC_NUMERO.Value;
    AsignarDatos(qryPrestamoscodigo_cte.AsString,
                 '',
                 qryPrestamosRNC_NUMERO.Value,
  ...
  begin
    ...
    xCedula := qryClienteStdRNC_NUMERO.Value;
...
begin
  ...
  begin
    if Not qryPrestamos.Locate('NOMBRE_FACTURAR', Edit1.Text,[loCaseInsensitive,loPartialKey]) then
  ...
  begin
    if Not qryPrestamos.Locate('NOMBRE_ABREV', Edit1.Text,[loCaseInsensitive,loPartialKey]) then
    MessageDlg('Nombre abreviado no encontrado',mtInformation, [mbOK], 0);
  end;
end;
...
begin
  ...
  qryClienteStd.SQL.Text:= Format(IBQuery1.Text + ' WHERE UPPER(%s) like %s %s',[Campo, Valor, OrderBy]);
end;
...
begin
  ...
  begin
    //xSerie := qryClienteStdSERIE.Value;
    codigoCliente:=qryClienteStdCodigo_cte.Value;
    xCodigoCte:=qryClienteStdCodigo_cte.Value;
    xCedula := qryClienteStdRNC_NUMERO.Value;
    AsignarDatos(qryClienteStdcodigo_cte.AsString,'',
    qryClienteStdRNC_NUMERO.Value,
    qryClienteStdNOMBRE_cte.Value,
