...
begin
  ...
  frmLogError.CurrenLN:=716;
  frmLogError.LogSteps(GlbNumVtaPOS,0,now,now,
  'Entrada a metodo dmCalculos.ProcInsertUpdatePosExtraDet',
  'UDatModCalculos');
  ...
  frmLogError.CurrenLN:= 809;
  frmLogError.LogSteps(GlbNumVtaPOS,serierxVenta,now,now,
  '-ignorado Antes qryConsultaPosExtraDet.Open;',
  'UDatMotCalculos');

  {experimento DGII rapidez
  if  datCambio then
  qryConsultaPosExtraDet.Open;//qryConsultaPosExtraDet.recordCount
  }

  frmLogError.LogSteps(GlbNumVtaPOS,serierxVenta,now,now,
  '-ignorado Despues qryConsultaPosExtraDet.Open;',
  'UDatModulos');

  frmLogError.CurrenLN:= 820;
  frmLogError.LogSteps(GlbNumVtaPOS,serierxVenta,now,now,
  'Actualizar totales',
  'UDatModModulos');
  ...
  if (GlbActivaIFiscal = 1) or (GlbMSubTotalSinITBIS = 1) then
  ...
  frmLogError.CurrenLN:= 853;
  frmLogError.LogSteps(GlbNumVtaPOS,serierxVenta,now,now,
  'tabla rxTotales actualizados',
  'UDatModModulos');

  frmLogError.CurrenLN:=858;
  frmLogError.LogSteps(GlbNumVtaPOS,0,now,now,
  'Salida del metodo dmCalculos.ProcInsertUpdatePosExtraDet',
  'UDatModCalculos');
  GlbCalculado:=True;
  avoidLoopExtra:=True;
  GlbExtraUpdate:=True;
...
begin
  ...
  if frmProcVentaRapida.rxVentacodArticulo.isnull then exit;
  frmLogError.CurrenLN:= 947;
  frmLogError.LogSteps(GlbNumVtaPOS,serierxVenta,now,now,
  'Entrada a  dmCalculos.Pos_UpdateTotales(GlbNumVtaPOS);',
  'UDatModCalculos');
  ...
  if (GlbActivaIFiscal = 1) or (GlbMSubTotalSinITBIS = 1) then                  //antes qryConsultaPosExtraDet
  frmProcVentaRapida.TotalesSubTotalScreen.Value:=qryGetSumVtaExtraTC_SUBTOTAL.Value
  else
  if (GlbMSubTotalSinITBIS = 0) then
  frmProcVentaRapida.TotalesSubTotalScreen.Value:=Abs(qryValorVtaMONTOBRUTO.Value);
  ...
  frmProcVentaRapida.Totales.Post;
  DatCambio:=False;
  //frmLogError.CurrenLN:= 1103;
  //frmLogError.LogSteps(GlbNumVtaPOS,serierxVenta,now,now,
  //'Saida de dmCalculos.Pos_UpdateTotales(GlbNumVtaPOS);',
  //'UDatModCalculos');
end;
