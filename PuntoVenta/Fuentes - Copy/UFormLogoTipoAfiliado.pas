...
type
  TfrmLogoTipoAfiliado = class(TForm)
    ...
    procedure tblLogoServicioAfterScroll(DataSet: TDataSet);
  private
...
procedure TfrmLogoTipoAfiliado.tblLogoServicioAfterScroll(
  DataSet: TDataSet);
begin
  if not tblLogoServicioRUTA_LOGO.IsNull then
  begin
    if FileExists(tblLogoServicioRUTA_LOGO.Value) then
    Image1.Picture.LoadFromFile(tblLogoServicioRUTA_LOGO.Value);
  end;
end;
