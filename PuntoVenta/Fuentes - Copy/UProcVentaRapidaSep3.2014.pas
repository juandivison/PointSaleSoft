...
{  Height := Height * Screen.Height Div Alto;
  ...
  dmFactura.ibQryViewNCF.SQL.Text := 'select v.*, 0 as TIPO_NCF_IFISCAL  from view_nfc v';
  if Not dmFactura.ibQryViewNCF.Prepared then
  dmFactura.ibQryViewNCF.Prepare;

  //dmFactura.ibQryViewNCFncfTIPO_NCF_IFISCAL.Value;
  //dmFactura.ibQryViewNCFNCF_TIPO_IFISCAL.Value;
...
begin
  ...
  Neto:=Neto  + TotalesRecargo.Value;
  rxVenta.EnableControls;
  ...
  CalcularDescIndirecto;
                                                                                           //aqui
  Totalesitbis.Value        := Itbis+ (TotalesRecargo.Value *
  FGlbPorcItbi(ExtraerFecha(rxVentaFecha.Value), -1)/
                       (1+FGlbPorcItbi(ExtraerFecha(rxVentaFecha.Value), -1)/100))/100;//-1 = buscar global
  Totalesmontodesc.Value    := Descuento;
  Totalestotalneto.Value := Totalestotalneto.Value;
...
begin
  ...
  begin
    Totales.Edit;
    TotalesMontoDescAdicional.Value:= (TotalesSubTotal.Value * TotalesPorcDesAdicional.Value / 100);
    Totales.Post;                            //aquisssssssssssssss
    if rxVenta.State = dsBrowse then
    rxVenta.Edit;
  ...
  begin
   ...
   dmfactura.qryFactReimpDet.Close;
    ...
    try
      ...
      dmReportes.qryDatosVentaMaster.Params[0].Value:= dmFactura.qryVentaFacturaNumero.Value;
    ...
    begin
      ...
      IntToStr(VarUsuarioGlb),IntToStr(dmFactura.qryDatosTrncxcNUMERO_DOC.Value),'.txt']);
      PrepararInfoFiscalNCR(dmFactura.qryDatosTrncxcNUMERO_DOC.Value);dmFactura.qryDatosTrncxc.params[0].value;
      ...
      if (dmfactura.qryGetNcfAsignadoNUMERO_NCF_AFECTADO.Isnull) then
      begin
        MessageDlg('Se requiere NCF afectado para Nota de Crédito, verifique.', mtError, [mbOk], 0);
        Exit;
      end;
      if (dmReportes.qryDatosRepTranscxcIFiscal.RecordCount = 0) then
      begin
        MessageDlg('Nota de Crédito no disponible para impresión, verifique.', mtError, [mbOk], 0);
        Exit;
      end else
      begin
        frmTestImpresoraFiscal.ncfAfectado := dmfactura.qryGetNcfAsignadoNUMERO_NCF_AFECTADO.Value;
 ...
 //{*****EN PRUEBA
  ...
  begin
    ...
    begin
      ...
      GlbNombreFiletoPrint:= Format('%s%s%s%s%s%s%s',[dmReportes.qryDatosCajaImpUserRUTA_TICKET.Value,'\NotaCredito',Trim(GlbCodSucursal),Trim(GlbModeloImpresora),
      IntToStr(VarUsuarioGlb),IntToStr(dmFactura.qryDatosTrncxcNUMERO_DOC.Value),'.txt']);
      PrepararInfoFiscalNCR(dmFactura.qryDatosTrncxcNUMERO_DOC.Value);
      if (dmReportes.qryDatosRepTranscxcIFiscal.RecordCount = 0) then
      begin
        MessageDlg('Nota de Crédito no disponible para impresión, verifique.',mtError, [mbYes,mbNo], 0);
        Exit;
      end else
      begin
        frmTestImpresoraFiscal.btnImprimeReciboClick(Self);
...
begin
  ...
  begin
   if MessageDlg('Ultima operacion fiscal, presentó un fallo. Cancelar ahora?',mtInformation,[mbyes, mbno],0) = mryes then
...
begin
  ...
  stpProc_UpdateVENTAS_MAST_U1.Params[31].Value:= TotalesRecargo.Value;  
...
begin
  //archivo:='C:\Proyectos\IFiscal\VB\Epsontm6iii\Epsontm6iii\bin\Release\Epsontm6iii.exe'; // if.bat - Shortcut';
  ShellExecute(0, 'Open', PChar(GlbExeIFiscal), PChar('/PortComm=4 /Baudrate=9600 /TipoDoc=DVFN /Status=not /Cancelar=True /CierreZ=False /CierreX=False'), PChar(''), SW_Hide);
...
begin
  //archivo:='C:\Proyectos\IFiscal\VB\Epsontm6iii\Epsontm6iii\bin\Release\Epsontm6iii.exe'; // if.bat - Shortcut';
  ShellExecute(0, 'Open', PChar(GlbExeIFiscal), PChar('/PortComm=4 /Baudrate=9600 /TipoDoc=DVFN /Status=not /CierreZ=True,89 /CierreX=False'), PChar(''), SW_Hide);
  ProcesaEstadoImpresora;
end;
...
begin
  //Cierre X
  //archivo:='C:\Proyectos\IFiscal\VB\Epsontm6iii\Epsontm6iii\bin\Release\Epsontm6iii.exe'; // if.bat - Shortcut';
  cmd:= '/PortComm=4 /Baudrate=9600 /TipoDoc=DVFN /Status=ifOnlineN /Caja=0001 /CierreZ=False,89  /CierreX=False /Cierre=X /Cancelar=False';
...
begin
  ...
  begin
    ...
    TotalesTipoNCF.Value := dmFactura.ibQryViewNCFTIPO_CF.Value;
    if Pos(UpperCase(dmFactura.ibQryViewNCFDESCRIPCION.Value), 'EXONERA') > 0 then
    begin
