...
type
  TfrmProcVentaRapidaNueva = class(TForm)
    Panel1: TPanel;
    BitBtn1: TBitBtn;
    BitBtn2: TBitBtn;
    BitBtn3: TBitBtn;
...
var
  frmProcVentaRapidaNueva: TfrmProcVentaRapidaNueva;
  TipoVenta : String;
  xCodigoCte : Integer;
...
uses UDatModConectar, UGlobal, URepComprobante, UDatModCompania,
  ...
  UConsultaPreventa, URepComprobanteNuevo;
...
procedure TfrmProcVentaRapidaNueva.BitBtn4Click(Sender: TObject);
var
...
begin
  ...
  begin
    ipStpInsertVentMast.Params[5].Value   := 'Devolución';
  ...
  begin
    ...
    qckRepReciboNuevo:=TqckRepReciboNuevo.Create(Nil);
    try
      qckRepReciboNuevo.ncfDesc := GlbDescNCF;
      if Length(edtObservacion.Text) > 0 then
      qckRepReciboNuevo.nombreCteGenerico:=edtObservacion.Text;
      qckRepReciboNuevo.xTotalesNombreCliente:=edtObservacion.Text;
      if (tablaPropietarioCodigoPropietario.value > 0) then
      begin
        qckRepReciboNuevo.qrLabelRNC.Caption:='RNC-' + qryClienteRNC_NUMERO.Value;
        qckRepReciboNuevo.nombreCteGenerico:= qryClienteNOMBRE_CTE.Value;
      end
      else qckRepReciboNuevo.qrLabelRNC.Caption:='';
      //RxDBGrid1.Visible:=False;
      //RxDBGrid1.DataSource:=DataSource2;
      qckRepReciboNuevo.xtipoVenta := TipoVenta;
      qckRepReciboNuevo.valorNCF:= valorNCF;
      pulgadaInc:=0.86;
      //temporal
      //{
      if rxVenta.RecordCount = 1 then
      qckRepReciboNuevo.Page.Length:= 5
      else
      if rxventa.RecordCount > 1 then
      begin
        flag:=false;

        qckRepReciboNuevo.Prepare;
        repeat
          if (qckRepReciboNuevo.PageNumber > 1) then
          begin
            qckRepReciboNuevo.Page.Length:=qckRepReciboNuevo.Page.Length +
            (rxventa.RecordCount -1) * 0.12;
            qckRepReciboNuevo.Prepare;
          end else flag:=true;
        until flag;
      end;
      ...
      begin
        qckRepReciboNuevo.qckRepComprobante.Size.Height := 0.250;
        qckRepReciboNuevo.QRLabel26.Enabled := False;
        qckRepReciboNuevo.QRDBText20.Enabled:= False;
      end;
      if (GlbImpReciboSinPreg = 1) and (GlbImprimeReciboFact = 1) then
      qckRepreciboNuevo.Print
      else
      if MessageDlg('Imprimir?',mtInformation, [mbYes, mbNo], 0)=mryes then
      begin
        if (GlbImprimeReciboFact = 1) then
        begin
          qckRepReciboNuevo.PrinterSetup;
          qckRepReciboNuevo.Print;
        end;
      end else
      qckRepReciboNuevo.PreviewModal;
      reciboImpreso:=True;
    finally
    qckRepReciboNuevo.Free;
    qckRepRecibo:=Nil;
    end;
    //temporal
    ImprimirFactura(100);
...
procedure TfrmProcVentaRapidaNueva.ProcActUltTrn;
begin
...
procedure TfrmProcVentaRapidaNueva.FormCreate(Sender: TObject);
begin
  sumarDatos:=True;
...
procedure TfrmProcVentaRapidaNueva.edtCodigoChange(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.edtCodigoExit(Sender: TObject);
var
  numero : Integer;
begin
...
procedure TfrmProcVentaRapidaNueva.ProcesaNuevo;
var
  numero : Integer;
begin
  if rxVenta.State In [dsInsert, dsEdit] then
...
procedure TfrmProcVentaRapidaNueva.BitBtn1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.BitBtn3Click(Sender: TObject);
begin
  if rxVenta.RecordCount > 0 then
  begin
...
procedure TfrmProcVentaRapidaNueva.DBEdit1Exit(Sender: TObject);
//var
...
procedure TfrmProcVentaRapidaNueva.BitBtn2Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.rxVentaAfterPost(DataSet: TDataSet);
//var
//  GuardarRec : TBookMark;
begin
//  GuardarRec := rxVenta.GetBookmark;
...
procedure TfrmProcVentaRapidaNueva.ProcesarSumaTotal;
var
  monto:Extended;
...
procedure TfrmProcVentaRapidaNueva.Edit1Exit(Sender: TObject);
var
  x,y : Real;
  Activa : Boolean;
  svalue : String;
...
procedure TfrmProcVentaRapidaNueva.edtCodigoEnter(Sender: TObject);
begin
  edtCodigo.SelectAll;
end;

procedure TfrmProcVentaRapidaNueva.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
  var
...
procedure TfrmProcVentaRapidaNueva.rxVentaBeforePost(DataSet: TDataSet);
begin
...
procedure TfrmProcVentaRapidaNueva.BitBtn5Click(Sender: TObject);
begin
  if rxVenta.state = dsbrowse then
  begin
    if VerificaClave then
...
procedure TfrmProcVentaRapidaNueva.Inventario1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.CuadreCaja1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.InsertarDet(numero : Integer);
begin
...
procedure TfrmProcVentaRapidaNueva.Ayuda1Click(Sender: TObject);
begin
  ShellExecute(frmProcVentaRapidaNueva.Handle,nil,PChar('C:\Proyectos\PointSaleSoftGold\PuntoVenta\ManualOnline.html'),'','',SW_SHOWNORMAL);
end;

procedure TfrmProcVentaRapidaNueva.ImprimirRecibo1Click(Sender: TObject);
begin
  ImprimirRecibo1.Checked:= Not ImprimirRecibo1.Checked;
end;

procedure TfrmProcVentaRapidaNueva.BitBtn6Click(Sender: TObject);
  var
     Serie_Asignadoncf : Integer;
     serieDoc : Integer;
     valorNCF : String;
...
procedure TfrmProcVentaRapidaNueva.BuscarCliente1Click(Sender: TObject);
begin
  FrmBuscarClientesPersonas:=TFrmBuscarClientesPersonas.Create(Nil);
  try
    xCodigoCte:=-1;
...
procedure TfrmProcVentaRapidaNueva.Cotizar1Click(Sender: TObject);
begin
  //BuscarCliente1Click(Self);
  BitBtn8Click(Self)
end;

procedure TfrmProcVentaRapidaNueva.BitBtn8Click(Sender: TObject);
begin
//tj   if Not Entro And GlbPermiteCambioPrecio then
//tj   begin
//tj     BitBtn10Click(Self);
...
//tj   end;
  ...
  try
    ...
    qckRepReciboNuevo.xtipoVenta := 'COTIZACION';
    qckRepReciboNuevo.valorNCF:= '';
    if rxVenta.RecordCount > 2 then
    qckRepReciboNuevo.Page.Length:= qckRepReciboNuevo.Page.Length +
    (rxVenta.RecordCount - 2 ) * 0.65;
    if (GlbImpReciboSinPreg = 1) then
    qckRepReciboNuevo.Print else
    if ImprimirRecibo1.Checked then
    begin
      qckRepReciboNuevo.PrinterSetup;
      qckRepReciboNuevo.Print;
    end  else
    qckRepReciboNuevo.Preview;
  finally
  qckRepReciboNuevo.Free;
  qckRepRecibo:=Nil;
  end;
...
procedure TfrmProcVentaRapidaNueva.InsertarCotizacionDet(numero: Integer);
begin
...
procedure TfrmProcVentaRapidaNueva.ProcesaCalculos;
var
  porItbi: Real;
begin
  rxVenta.AutoCalcFields:=False;
...
procedure TfrmProcVentaRapidaNueva.BitBtn9Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.ProcCargaDatosCotizacion;
begin
...
procedure TfrmProcVentaRapidaNueva.ProcesaFacturar(tipo : smallint; numdoc:Integer; codcte:Integer);
begin
...
procedure TfrmProcVentaRapidaNueva.ActBalaceCteP(operacion:smallint);
var
  Monto:Real;
begin
  if (Operacion=2) then
...
procedure TfrmProcVentaRapidaNueva.EditarBalanceP(monto:Real);
begin
...
procedure TfrmProcVentaRapidaNueva.EditarBalanceP(monto:Real;tipo:Integer);
begin
  dmcxc.qryBalanceCtes.Edit;
...
procedure TfrmProcVentaRapidaNueva.InsertaBalanceP(monto:Real);
begin
...
procedure TfrmProcVentaRapidaNueva.ProcVentaConNCF(tiponcf:String;var ValorNCF:String;var seriedoc:Integer; var serie_asignadoncf : Integer);
begin
  //Generar y Asignar Numero de Comprobante Fiscal
  valorNCF:='';
...
procedure TfrmProcVentaRapidaNueva.MaestroCliente1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.RxDBGrid2DblClick(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.DBEdit1Enter(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.BitBtn10Click(Sender: TObject);
begin
  if (rxVenta.RecordCount >  0) then
  begin
    Totales.Edit;
...
procedure TfrmProcVentaRapidaNueva.DBEdit5Exit(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.BitBtn11Click(Sender: TObject);
var
  frecuencia:string;
  serieDoc:Integer;
...
function TfrmProcVentaRapidaNueva.FncFrecuencia(Frec: Integer): String;
begin
  case Frec of
  30 : result:='Mes(es)';
...
procedure TfrmProcVentaRapidaNueva.BitBtn12Click(Sender: TObject);
var
 serieDoc:Integer;
 frecuencia:string;
 ValorNCF : String;
...
procedure TfrmProcVentaRapidaNueva.MostrarVistaproductos1Click(Sender: TObject);
begin
  MostrarVistaproductos1.Checked := Not MostrarVistaproductos1.Checked;
end;

procedure TfrmProcVentaRapidaNueva.BitBtn13Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.edtReferenciaChange(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.edtReferenciaExit(Sender: TObject);
begin
  if Panel2.Visible then
...
procedure TfrmProcVentaRapidaNueva.NotaCrdito1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.NotaDebito1Click(Sender: TObject);
begin
 { frmTransCxcNCdbDev:=TfrmTransCxcNCdbDev.Create(Nil);
  try
...
procedure TfrmProcVentaRapidaNueva.BitBtn14Click(Sender: TObject);
begin
  DBEdit2.ReadOnly := False;
...
procedure TfrmProcVentaRapidaNueva.DBEdit2Exit(Sender: TObject);
var
  guardarRec : TBookMark;
  porcdes : Extended;
begin
...
function TfrmProcVentaRapidaNueva.VerificaClave: Boolean;
begin
...
procedure TfrmProcVentaRapidaNueva.Devolucin1Click(Sender: TObject);
begin
  frmProcVentaRapidaNueva.Hide;
...
procedure TfrmProcVentaRapidaNueva.BitBtn7Click(Sender: TObject);
var
  x : Integer;
begin
...
procedure TfrmProcVentaRapidaNueva.DBEdit2Enter(Sender: TObject);
begin
  if (GlbNumero > 0) then
  if rxVenta.State = dsbrowse then
...
procedure TfrmProcVentaRapidaNueva.chkExentoExit(Sender: TObject);
begin
  if (rxVenta.Recordcount > 1) then
  begin
    SumarDatos := False;
...
procedure TfrmProcVentaRapidaNueva.RxDBGrid2KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = '#13' then
...
procedure TfrmProcVentaRapidaNueva.RxDBGrid2KeyDown(Sender: TObject;
...
procedure TfrmProcVentaRapidaNueva.AsignarSqlText(campo, valor,
  OrderBy: String);
var
  nombProc:string;
begin
...
procedure TfrmProcVentaRapidaNueva.chkExentoEnter(Sender: TObject);
begin
  chkExento.SetFocus;
end;

procedure TfrmProcVentaRapidaNueva.RxDBGrid2GetCellParams(Sender: TObject;
...
procedure TfrmProcVentaRapidaNueva.ReImprimirRecibo1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.BitBtn15Click(Sender: TObject);
var
  tRecordsDet : integer;
begin
  dmCompania.tblCompania.Close;
...
procedure TfrmProcVentaRapidaNueva.FormShow(Sender: TObject);
begin
  ProcActUltTrn;
  isShowing:=True;
end;

procedure TfrmProcVentaRapidaNueva.RxLabel1Click(Sender: TObject);
begin
  ProcActUltTrn;
end;

procedure TfrmProcVentaRapidaNueva.ProcVenta(valorNCF:String;Serie_Asignadoncf:Integer; Var serieDoc:Integer);
begin
  if (seriedoc <= 0 ) then
  begin
    serieDoc:= FsqlMaxNumero('facturas','numero');
  ...
  begin
    ...
    begin
      ...
      qckRepReciboNuevo.Free;
      qckRepReciboNuevo:=Nil;
      end;
    end;
  end;
end;

procedure TfrmProcVentaRapidaNueva.LectorCodBarra1Click(Sender: TObject);
begin
  LectorCodBarra1.Checked:= Not LectorCodBarra1.Checked;
...
procedure TfrmProcVentaRapidaNueva.Edit1Enter(Sender: TObject);
begin
  if (Totalestotalneto.Value < 0) then
  begin
...
procedure TfrmProcVentaRapidaNueva.ImprimirLabel1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.NoGenerarNCF1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.BitBtn16Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.Finalizardisplay;
var
  puerto, linea1, linea2 : String;
begin
  puerto:='1';
...
procedure TfrmProcVentaRapidaNueva.ActualizaInvAlmacen;
begin
...
procedure TfrmProcVentaRapidaNueva.FormHide(Sender: TObject);
begin
  isShowing:=True;
end;

procedure TfrmProcVentaRapidaNueva.qryClienteAfterScroll(DataSet: TDataSet);
begin
  AbrirClasifPrecio;
end;

procedure TfrmProcVentaRapidaNueva.qryProductosAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmProcVentaRapidaNueva.AbrirClasifPrecio;
begin
...
procedure TfrmProcVentaRapidaNueva.AplicaPrecioMayorista;
begin
...
procedure TfrmProcVentaRapidaNueva.RxDBLookupCombo1Exit(Sender: TObject);
begin
  AbrirClasifPrecio;
  AplicaPrecioMayorista;
end;

function TfrmProcVentaRapidaNueva.VerificaLimiteCredito(codigocte: integer;
...
procedure TfrmProcVentaRapidaNueva.FormCloseQuery(Sender: TObject;
...
procedure TfrmProcVentaRapidaNueva.RxDBGrid3DblClick(Sender: TObject);
begin
  RxDBGrid3.Visible:=false;
end;

procedure TfrmProcVentaRapidaNueva.SecretPanel1DblClick(Sender: TObject);
begin
  RxDBGrid3.Visible:= True;
  SecretPanel1.Visible:=False;
end;

procedure TfrmProcVentaRapidaNueva.NoMostrarProdVencidos1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.rxLabelVenceDblClick(Sender: TObject);
begin
  RxDBGrid3.Visible:= True;
end;

procedure TfrmProcVentaRapidaNueva.cboxTipoUnidadChange(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.LlenarTipoUnidadBox(umedida:integer);
begin
  dmInventario.qryTipoUnidad.First;
...
procedure TfrmProcVentaRapidaNueva.cboxTipoUnidadExit(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.qryInventarioAfterScroll(DataSet: TDataSet);
begin
...
function TfrmProcVentaRapidaNueva.precioXUnidad(idTipoUnidad:Integer;idProducto: Integer): Extended;
begin
...
procedure TfrmProcVentaRapidaNueva.Label25Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.RxSpeedButton1Click(Sender: TObject);
begin
  GlbImprimeReciboFact:=0;
  BitBtn4Click(Self);
...
procedure TfrmProcVentaRapidaNueva.MaestroInventario1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.ImprimirFactura(Avance: Integer);
...
procedure TfrmProcVentaRapidaNueva.cboxTipoUnidadEnter(Sender: TObject);
begin
  panel2.Visible := False;
end;

procedure TfrmProcVentaRapidaNueva.DBEdit3Exit(Sender: TObject);
begin
  if (rxVentaMontoBruto.Value = 0) or (rxVentaMontoBruto.IsNull) then
  edtCodigo.SetFocus;
end;

procedure TfrmProcVentaRapidaNueva.RxDBGrid2Enter(Sender: TObject);
begin
  GlbNumero:= rxVentaSerie.Value;
end;

procedure TfrmProcVentaRapidaNueva.RxDBGrid2Exit(Sender: TObject);
begin
  GlbNumero:= rxVentaSerie.Value;
...
procedure TfrmProcVentaRapidaNueva.rxVentaAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmProcVentaRapidaNueva.ConsultaGeneral1Click(Sender: TObject);
begin
...
procedure TfrmProcVentaRapidaNueva.BitBtn17Click(Sender: TObject);
begin
  if (Totalesmontorecibido.Value > 0) and(rxVenta.RecordCount > 0) then
  begin
    MessageDlg('Favor procesar o cancelar venta en progreso', mtInformation, [mbOK], 0);
...
procedure TfrmProcVentaRapidaNueva.CargarPreventa;
begin
...
procedure TfrmProcVentaRapidaNueva.ActualizarPreventa;
begin
