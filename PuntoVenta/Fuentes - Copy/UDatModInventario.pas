...
type
  TdmInventario = class(TDataModule)
    ...
    procedure qryProveedoresFilterRecord(DataSet: TDataSet;
      var Accept: Boolean);
    procedure tblInventarioProdAfterScroll(DataSet: TDataSet);
  private
...
begin
  ...
  Accept:=Dataset['Tipo'] = filterTipoInv;
end;
...
procedure TdmInventario.tblInventarioProdAfterScroll(DataSet: TDataSet);
begin
  //temporal
  {if dmInventario.tblInventarioProd.State in [dsInsert, dsEdit] then exit;

  qryInvOtro.Close;
  qryInvOtro.Params[0].Value:=tblInventarioProdCODIGO.Value;
  qryInvOtro.Open;
  }
end;
