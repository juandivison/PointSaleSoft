...
interface
 Uses Forms,Dbtables, db, Sysutils, windows, Dialogs, Controls,Printers,Classes,
    ...
    Math, VCLUnZip, VCLZip, StdCtrls, Registry, OleServer, ExcelXP, comObj, ComCtrls;
 const
 ...
 var
    ...
    GLBImpComAdic:integer;
    GlbValorMaxVtaWarning:real;
  ...
  nombrearchivo:string);
   ...
   function Desencriptar(const S: String; Key: Integer): String;
   procedure PatchINT3;
...
specialfolders, UDatmodPerm, UDatModReportes, UDatamodulocnt, UDatModInventario, UDatmodDatosGenerales, SynPdf, UDatModNomina;
...
begin
  ...
  F.Position := F.Size;
  s := FormatDatetime('yyyy-mm-dd hh:mm:ss',now)+' '+ linea + #13 + #10;
...
begin
   ...
   s2:=inttostr(RandomRange(1000, 9999));
...
begin
  ...
  try
    ...
    ShellExecute(0, 'Open', PChar(archivo), PChar(parameters), PChar(''), SW_Hide);
     ...
     //raise Exception.Create('Error contactando impresora fiscal, verifique');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          descX,
          VarUsuarioGlb,'A','', Now,strusername,FormatDateTime('dd/mm/yyyy hh:mm a/p',now),strusername,glbZipFile,dmCompania.tblCompaniaEMAIL.Value);
        end;
        GlbEnviaEmail:=False;
      end;
    end;
...
function Desencriptar(const S: String; Key: Integer): String;//Word
var
  I: byte;
begin
  if (s = '') then result:='';
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          ''#13#10'Reporte',
          VarUsuarioGlb,'A','',now,strusername,FormatDateTime('dd/mm/yyyy hh:mm a/p',now),strusername,targetFile,dmCompania.tblCompaniaEMAIL.Value);
        end;
      end;
      GlbEnviaEmail:=False;
