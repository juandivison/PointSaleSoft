...
uses UDatModFactura, uglobal, UDatModCxc, UFactServicio,
  uFormAsignarNCFVenta, UDatModCompania;
{$R *.dfm}
...
begin
 if dmFactura.tblProgServIguala.State = dsBrowse then
  begin
    ...
    dmFactura.tblProgServIgualaFECHA_INSERT.Value  := Now;
...
begin
  ...
  ipStpInsertVentMast.Params[16].Value:= dmfactura.tblProgServIgualaMONTO.Value;
