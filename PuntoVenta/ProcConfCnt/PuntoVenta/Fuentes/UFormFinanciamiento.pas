...
begin
    ...
    stpInsFinanciamiento.Params[7].Value := rxDatosFinancMontoFinanciado.Value;//rxDatosFinancMontoInicial.Value;
    stpInsFinanciamiento.Params[8].Value := rxDatosFinancMontoFinanciado.Value;
    stpInsFinanciamiento.Params[9].Value := 0;
    stpInsFinanciamiento.Params[10].Value:= 0;
    stpInsFinanciamiento.Params[11].Value:= 0;
    stpInsFinanciamiento.Params[12].Value:= rxDatosFinancMontoFinanciado.Value;
    ...
    stpInsFinanciamiento.Params[22].Value:= rxDatosFinancMontoInicial.Value;
...
procedure TfrmFinanciamiento.BitBtn7Click(Sender: TObject);
begin
{  frmConsultaCertificacion:=TfrmConsultaCertificacion.Create(Nil);
  try
    if frmConsultaCertificacion.Showmodal = mrok then
    begin
      dmCertificacion.tblCertificacion.Close;
      dmCertificacion.tblCertificacion.params[0].value:= ExtraerFecha(frmConsultaCertificacion.qryConsultaCertifecha.Value);
      dmCertificacion.tblCertificacion.params[1].value:= ExtraerFecha(frmConsultaCertificacion.qryConsultaCertifecha.Value);
      dmCertificacion.tblCertificacion.Open;
      dmCertificacion.tblCertificacion.locate('cedula', trim(frmConsultaCertificacion.qryConsultaCertiCedula.Value),[]);
    end;
  finally
  frmConsultaCertificacion.Free;
  frmConsultaCertificacion:=Nil;
  end;}
end;

procedure TfrmFinanciamiento.BitBtn8Click(Sender: TObject);
begin
{  qckRepCertificacion:=TqckRepCertificacion.Create(Nil);
  try
    qckRepCertificacion.Preview;
  finally
  qckRepCertificacion.Free;
  qckRepCertificacion:=Nil;
  end;}
end;
...
begin
  ...
  (rxDatosFinancPorcInteres.Value/100) * rxDatosFinancMontoInicial.Value;

  rxDatosFinancMontoFinanciado.Value := rxDatosFinancMontoInicial.Value +
