...
uses
  ...
  OleServer, ExcelXP, RXSpin, IBTable, rxToolEdit, ComCtrls;
...
uses UEdicionVenta, URepRecibo8_5_11Devol, URepComprobanteDevol, uGlobal,
  UFormSelSerieProd, URepRecibo8_5_11DevolREspinal;
{$R *.dfm}
...
begin
  ...
  if rxSolDatoscte.State In [dsInactive] then
    ...
    begin
      qckRepRecibo8_5_11DevolResp :=TqckRepRecibo8_5_11DevolResp.Create(Nil);
      try
        //qckRepRecibo8_5_11DevolResp.TPag.Caption:= IntToStr(qckRepRecibo8_5_11DevolResp.PageNumber);
        qckRepRecibo8_5_11DevolResp.Prepare;
        glbTPag := qckRepRecibo8_5_11DevolResp.PageNumber;

      finally
      qckRepRecibo8_5_11DevolResp.Free;
      qckRepRecibo8_5_11DevolResp:=Nil;
      end;

      qckRepRecibo8_5_11DevolResp:=TqckRepRecibo8_5_11DevolResp.Create(Nil);
      try
        qckRepRecibo8_5_11DevolResp.xtipoVenta := TipoVenta;
        qckRepRecibo8_5_11DevolResp.valorNCF   := valorNCF;
        qckRepRecibo8_5_11DevolResp.Preview;
        if MessageDlg('Re-imprimir',mtInformation, [mbNo, mbyes], 0) = mryes then
        begin
          qckRepRecibo8_5_11DevolResp.Preview;
        end;
  ...
  begin
    ...
    try
      ...
      begin
        qckRepReciboDevol.QRLabel27.Enabled := False;
        qckRepReciboDevol.QRDBText5.Enabled:= False;
      end;
      ...
      if not frmEdicionVenta.tblDatosVentaNOMBRECLIENTE.IsNull then
      qckRepReciboDevol.nombreCteGenerico:=frmEdicionVenta.tblDatosVentaNOMBRECLIENTE.Value
      else
      if Length(edtObservacion.Text) > 0 then
      ...
      qckRepReciboDevol.Page.Length:= 5
      else 
      ...
      repeat
        begin
         qckRepReciboDevol.Prepare;
          if qckRepReciboDevol.PageNumber > 1 then
          begin
            qckRepReciboDevol.Page.Length := qckRepReciboDevol.Page.Length + 0.25;
            qckRepReciboDevol.Prepare;
            flag:=False;
          end else break;
        end;
...
procedure TfrmProcVentaRapidaEdicion.qryInventarioAfterOpen(
  DataSet: TDataSet);
begin
  inherited;
//
...
procedure TfrmProcVentaRapidaEdicion.rxVentaAfterScroll(DataSet: TDataSet);
begin
  inherited;
  //
...
procedure TfrmProcVentaRapidaEdicion.rxVentaAfterPost(DataSet: TDataSet);
begin
  inherited;
  //
end;

procedure TfrmProcVentaRapidaEdicion.rxVentaBeforePost(DataSet: TDataSet);
begin
  inherited;
  //
end;

procedure TfrmProcVentaRapidaEdicion.BitBtn10Click(Sender: TObject);
begin
  inherited;
 //
end;
