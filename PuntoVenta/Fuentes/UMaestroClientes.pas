...
type
  TfrmClientes = class(TForm)
    ...
    DBText3: TDBText;
    DBText4: TDBText;
    btnValidarDGI: TBitBtn;
    DBRadioGroup5: TDBRadioGroup;
    FiltrarNoTranspITBIS1: TMenuItem;
    NoTranspITBISTodos1: TMenuItem;
    SiTranspITBISTodos1: TMenuItem;
    ...
    procedure rxdblookupIDMunicipioExit(Sender: TObject);
    ...
    procedure btnValidarDGIClick(Sender: TObject);
    procedure FiltrarNoTranspITBIS1Click(Sender: TObject);
    procedure RxLabel1Click(Sender: TObject);
    procedure NoTranspITBISTodos1Click(Sender: TObject);
    procedure SiTranspITBISTodos1Click(Sender: TObject);
  private
    ...
    procedure ProcActualizaTranspItbis(tipo: smallint);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (GlBAyaco = 1) then
      dmClientes.tblClientesSHOWPRECIOCONITBIS.Value := 1
      else
      dmClientes.tblClientesSHOWPRECIOCONITBIS.Value := 0;
...
begin
  ...
  begin 
    ...
    begin
      ...
      if (dmClientes.tblClientesCANT_DIAS_CREDITO.IsNull and
          (dmClientes.tblClientesCONDICION.Value = 2)) then
      begin
        MessageDlg('Debes indicar la cantidad días de crédito.', mtError,[mbOk],0);
        DBComboBox1.SetFocus;
      end;
      if (not dmClientes.tblClientesCANT_DIAS_CREDITO.IsNull and
          ((dmClientes.tblClientesLIMITE_CREDITO.Value = 0)
           or (dmClientes.tblClientesLIMITE_CREDITO.IsNull))
         and (dmClientes.tblClientesCONDICION.Value = 2)) then
      begin
        MessageDlg('Debes indicar el límite de crédito.', mtError,[mbOk],0);
        DBEdit6.SetFocus;
      end;
      ...
      begin
        if dmClientes.tblClientesID_PROVINCIA.IsNull or dmClientes.tblClientesID_MUNICIPIO.IsNull then
        begin
          BitBtn54Click(Self);
        end;
      end;
  ...
  begin
    ...
    dmClientes.tblClientes.GotoBookmark(guardarRec);
    dmClientes.tblClientes.FreeBookmark(guardarRec);
...
begin
  dmClientes.FiltrarTranspItbis:=False;
  if GlbActivaECF = 0 then
  BitBtn54.Visible:=False else BitBtn54.Visible:=True;
  ...
  dmDatos.qryMunicipioseCF.Params[0].Value:=dmclientes.tblClientesID_PROVINCIA.Value;
  dmDatos.qryMunicipioseCF.Params[1].Value:=dmclientes.tblClientesID_PROVINCIA.Value;
...
var
 ...
 AInfo, AError: String;
begin
  if ((GlbActivaECF = 1) or (GlbActivaIFiscal = 1)) And (Length(dmClientes.tblClientesRNC_NUMERO.Value) > 0) then
  begin
  ...
  if dmclientes.tblClientes.state in [dsEdit, dsInsert] then
  begin
    if dmClientes.qryClientes.Locate('RNC_NUMERO',dmClientes.tblClientesRNC_NUMERO.Value,[]) then
    begin
      MessageDlg('Número: '+dmClientes.tblClientesRNC_NUMERO.Value+', existe. Verifique',mtError,[mbok],0);
    end;
    if (GlbActivaECF = 1) then
    btnValidarDGIClick(self);
  end;
...
procedure TfrmClientes.ProcOpenTerritorios;
begin
  if (dmDatos.qryMunicipioseCF.Params[0].Value <> dmclientes.tblClientesID_PROVINCIA.Value ) then
  begin
    dmDatos.qryMunicipioseCF.Close;           //dmclientes.tblClientesID_MUNICIPIO.VALUE;
    dmDatos.qryMunicipioseCF.Params[0].Value:=dmclientes.tblClientesID_PROVINCIA.Value;
    dmDatos.qryMunicipioseCF.Params[1].Value:=dmclientes.tblClientesID_PROVINCIA.Value;
    dmDatos.qryMunicipioseCF.Open;//dmDatos.qryMunicipioseCFNombre.Value
  end;
end;


procedure TfrmClientes.rxdblookupIDMunicipioExit(Sender: TObject);
begin
  if (GlbActivaECF = 0) then exit;
  if not dmclientes.tblClientesID_PROVINCIA.IsNull and dmclientes.tblClientesID_MUNICIPIO.IsNull then
  ShowMessage('Asignar valor a municipio');
end;

procedure TfrmClientes.BitBtn54Click(Sender: TObject);
var
  guardarRec : TBookmark;
begin
  guardarRec := dmclientes.tblClientes.GetBookmark;
  ...
  if dmclientes.tblClientes.State = dsEdit then
  begin
    dmclientes.tblClientesFECHA_MOD.Value:=Now;
    GlbSalvarQuery(dmClientes.tblClientes);
  end;
  if Assigned(guardarRec) then
  begin
...
procedure TfrmClientes.DBEdit44DblClick(Sender: TObject);
begin
  BitBtn54Click(Self);
end;

procedure TfrmClientes.DBEdit43DblClick(Sender: TObject);
begin
  BitBtn54Click(Self);
end;

procedure TfrmClientes.RxDBLookupCombo1Change(Sender: TObject);
begin
  if dmClientes.tblClientes.State in [dsInsert, dsEdit] then
  begin
    if (dmClientes.tblClientesCONDICION.Value = 2) then
    begin
      dmClientes.tblClientesCANT_DIAS_CREDITO.Value:=30;
    end;
  end;
end;

procedure TfrmClientes.btnValidarDGIClick(Sender: TObject);
var
  Info   : TDgiiRncInfo;
  Error  : string;
  EsActivo: Boolean;
  Rnc    : string;
begin
  if dmClientes.tblClientesRNC_NUMERO.IsNull then exit;

  Rnc := Trim(dmClientes.tblClientesRNC_NUMERO.Value);        // el RNC escrito por el usuario
  if (Rnc = '') then exit;
...
procedure TfrmClientes.FiltrarNoTranspITBIS1Click(Sender: TObject);
begin
  if FiltrarNoTranspITBIS1.Checked then
  begin
    dmClientes.tblclientes.Filtered:=False;
    dmClientes.FiltrarTranspItbis := False;
  end else
  begin
    dmClientes.tblclientes.Filtered:=False;
    dmClientes.FiltrarTranspItbis := True;
    dmClientes.tblclientes.Filtered:=True;
    FiltrarNoTranspITBIS1.Checked :=True;
  end;
end;

procedure TfrmClientes.RxLabel1Click(Sender: TObject);
begin
 dmClientes.tblclientes.Filtered:=False;
 RxLabel1.Visible := dmClientes.tblclientes.Filtered;
end;

procedure TfrmClientes.NoTranspITBISTodos1Click(Sender: TObject);
begin
  ProcActualizaTranspItbis(1);
end;

procedure TfrmClientes.ProcActualizaTranspItbis(tipo:smallint);
var
 guardarRec : TBookmark;
begin
  guardarRec:= dmClientes.tblClientes.GetBookmark;
...
procedure TfrmClientes.SiTranspITBISTodos1Click(Sender: TObject);
begin
  ProcActualizaTranspItbis(0);
end;
