...
type
  ...
  public
    ...
    procedure ProcAsignareCF(numerotrn, serie: integer;showVNCF:Boolean);
    procedure ProcReAsignareCF(numerotrn, serie: integer;
    showVNCF:Boolean;_ncfGenerado:string;ncfAnterior:string;_tiponcf:string;var nuevoECF:string);
  end;
...
begin
  ...
  try
    ...
    else if not flag and not showNcfwindows and ((GlbActivaeCF= 0) or( GlbActivaIFiscal = 0)) then
    begin
      if frmSelNCF.ShowModal = mrOk then
      begin
        genNcf := True;
...
procedure TfrmAsignarNCFAVenta.ProcAsignareCF(numerotrn, serie: integer;showVNCF:Boolean);
var
  showNcfwindows, genNcf, flag: Boolean;
begin
  frmSelNCF := TfrmSelNCF.Create(Self);
...
procedure TfrmAsignarNCFAVenta.ProcReAsignareCF(numerotrn, serie: integer;
showVNCF:Boolean;_ncfGenerado:string;ncfAnterior:string;_tiponcf:string;var nuevoECF:string);
begin
  dmFactElectronica.tblCambiarNCF.Close;
  dmFactElectronica.tblCambiarNCF.Params[0].Value:=serie;
  dmFactElectronica.tblCambiarNCF.Open;
  dmFactElectronica.tblCambiarNCF.First;
  if not dmFactElectronica.tblCambiarNCFSERIE.IsNull then
  begin
    dmFactElectronica.tblCambiarNCF.Edit;
    dmFactElectronica.tblCambiarNCFTIPO_NCF.Value:=_tiponcf;
    dmFactElectronica.tblCambiarNCFFECHA_UPDATE.Value:=now;
    dmFactElectronica.tblCambiarNCFCODIGO_USUARIO_UPD.Value:=VarUsuarioGlb;
    if dmFactElectronica.tblCambiarNCFNCF_ANTERIOR.IsNull then
    dmFactElectronica.tblCambiarNCFNCF_ANTERIOR.Value:=ncfAnterior;
    dmFactElectronica.tblCambiarNCFNUMERO_NCF.Value:=_ncfGenerado;
    nuevoECF:=_ncfGenerado;
