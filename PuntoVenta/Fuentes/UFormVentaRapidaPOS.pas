...
begin
  ...
  begin
    ...
    begin
      ...
      if (imbBarcode.Visible = false ) then
      panel2.Visible := True;
      Panel2.Height  := 152;
      panel2.BringToFront;
    end
  ...
  begin
    ...
    begin
      ...
      tblPOSVentadetMONTODESCUENTO.Value:=0;
      tblPOSVentadetMONTODESGLOBAL.Value:=0;
      tblPOSVentadetMONTODESCGLBDIST.Value:=0;
      tblPOSVentadetMONTOITBRECITMCONDESC.Value:=0;
      tblPOSVentadetTIPOUNIDAD.Value:=1;
      tblPOSVentadetPORC_DESC.Value :=0;
...
begin
  ...
  //pos exito if tblPosVentaDet.State In [dsInsert, dsEdit] Then
//pos exito   GlbSalvarQuery(tblPOSVentadet);
  GlbNumeroSerie:= tblPOSVentadetSerie.Value;
  //pos exito if tblPosVentaDet.State = dsBrowse then
  //pos exito tblPosVentaDet.Locate('Serie', yNumSerie,[]);
...
begin
  ...
  GlbSalvarQuery(tblPOSTotales);
  dmcalculos.stpPROC_UpdatePosVentaDET.params[0].Value:= tblPOSVentadetNumero.Value;
  dmcalculos.stpPROC_UpdatePosVentaDET.ExecProc;
  dmCalculos.POSTRefrescaDet;
  if Not dmcalculos.stpPROC_UpdatePosVentaDET.Transaction.InTransaction then
  dmcalculos.stpPROC_UpdatePosVentaDET.Transaction.StartTransaction;
  try
    dmcalculos.stpPROC_UpdatePosVentaDET.Transaction.CommitRetaining;
  except
  dmcalculos.stpPROC_UpdatePosVentaDET.Transaction.RollbackRetaining;
  end;
...
begin
  dmCalculos.PosSumatotal;
    ...
    dmcalculos.qryPosExtraDET.Params[0].Value:= tblPOSVentadetNUMERO.Value;
    dmcalculos.qryPosExtraDET.Open;
    if tblPOSTotales.state = dsbrowse then
    tblPOSTotales.edit;

    tblPOSTotalesSUBTOTAL.Value:=
    dmcalculos.qryPosExtraDETTC_SUBTOTAL.Value;

    tblPOSTotalesMONTODESCITEMS.Value:=
    dmcalculos.qryPosExtraDETTTC_MONTODESCITEMS.Value;

    tblPOSTotalesITBIS.Value:= dmcalculos.qryPosExtraDETTC_MONTOITBISRECARGO_GLB.Value;


    if (tblPOSTotalesMONTOEXONERAITBIS.Value > 0) then
    tblPOSTotalesMONTOEXONERAITBIS.Value:=tblPOSVentadetITBIS_TOTAL.Value;
    //pos exito GlbSalvarQuery(tblPOSTotales);
    //pos exito dmCalculos.PosSumatotal;
    //pos exito tblPOSTotales.Close;
    //pos exito tblPOSTotales.Open;
  //end;
  //else
