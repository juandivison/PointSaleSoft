...
var
  ...
  procedure AppendAvailablePipeData;
  begin
    while PeekNamedPipe(hRead, nil, 0, nil, @BytesRead, nil) and (BytesRead > 0) do
    begin
      FillChar(Buffer, SizeOf(Buffer), 0);
      if ReadFile(hRead, Buffer, SizeOf(Buffer) - 1, BytesRead, nil) and (BytesRead > 0) then
      begin
        SetString(TempAnsi, PAnsiChar(@Buffer[0]), BytesRead);
        AOutput := AOutput + string(TempAnsi);
      end
      else
        Break;
    end;
...
begin
  ...
  try
    // El proceso hijo NO debe heredar el extremo de lectura
