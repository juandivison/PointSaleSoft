...
type
  TFacturaPickerMode = (fpmNormal, fpmNcrSelection);

  TfrmConsultaFacturas2Ecf = class(TForm)
    RxDBGrid1: TRxDBGrid;
    RxDBGrid2: TRxDBGrid;
  ...
  private
    ...
    FPickerMode: TFacturaPickerMode;
    FSelectedNumeroVenta: Integer;
    FSelectedNumeroFactura: Integer;
    FSelectedNcf: string;
    procedure ApplyPickerMode;
    procedure AcceptCurrentFacturaForNcr;
    ...
    function GetNcrNcfRefFilter: string;
    procedure ApplyNcrNcfRefFilter(var ASql: string);
    procedure SetParamIfExists(ADataSet: TIBQuery; const AParamName, AValue: string);
  public
    { Public declarations }
    EsReimprimirContado : Boolean;
    EsActualizaConduce : Boolean;
    NumTrnVenta : Integer;
    function ExecuteForNcrSelection: Boolean;
    property SelectedNumeroVenta: Integer read FSelectedNumeroVenta;
    property SelectedNumeroFactura: Integer read FSelectedNumeroFactura;
    property SelectedNcf: string read FSelectedNcf;
    ...
    procedure OpenVentaFacturaWithNcfFilter;
  end;

var
  FiltrarSoloMonto:Boolean = false;
  frmConsultaFacturas2Ecf: TfrmConsultaFacturas2Ecf;
...
USES UDatModFactura, uglobal, UDatModClientes, UFormCambiarCteFact,
  ...
  UFrmCartaRutaTemplate, UFrmCartaSaldoTemplate, UFrmOrders2EcfRunner,
  UFormFacturas;
{$R *.dfm}

//Idesi2ECF
type
  TNcrParamSnapshot = record
    Name: string;
    Value: Variant;
  end;

procedure TfrmConsultaFacturas2Ecf.OpenVentaFacturaWithNcfFilter;
var
  NcfRef: string;
  NcfDs: string;
  I: Integer;
  Snap: array of TNcrParamSnapshot;
  ASql:String;
begin
  NcfRef := Trim(GetNcrNcfRefFilter);

  if NcfRef <> '' then
  begin
    // IMPORTANTE:
    // Este helper puede llamarse después de que el código legacy ya asignó Params[0], Params[1], etc.
    // Al modificar SQL.Text, IBX reconstruye Params y puede perder valores ya asignados.
    // Por eso guardamos y restauramos los parámetros existentes antes de agregar :NCR_NCF_REF.
    SetLength(Snap, dmFactura.qryVentaFactura.Params.Count);
    for I := 0 to dmFactura.qryVentaFactura.Params.Count - 1 do
    begin
      Snap[I].Name := dmFactura.qryVentaFactura.Params[I].Name;
      Snap[I].Value := dmFactura.qryVentaFactura.Params[I].Value;
    end;

    ApplyNcrNcfRefFilter(ASql);

    for I := 0 to High(Snap) do
    begin
      if (Snap[I].Name <> '') and
         (dmFactura.qryVentaFactura.Params.FindParam(Snap[I].Name) <> nil) then
        dmFactura.qryVentaFactura.ParamByName(Snap[I].Name).Value := Snap[I].Value;
    end;
  end;

  SetParamIfExists(dmFactura.qryVentaFactura, 'NCR_NCF_REF', NcfRef);

  dmFactura.qryVentaFactura.Open;

  // Blindaje: si el runner indicó un e-NCF esperado, la consulta debe quedar exactamente ahí.
  if NcfRef <> '' then
  begin
    if dmFactura.qryVentaFactura.IsEmpty then
      raise Exception.CreateFmt(
        'No se encontró la factura con el e-NCF [%s]. No se puede continuar con la NCR.',
        [NcfRef]);

    if dmFactura.qryVentaFactura.FindField('NUMERO_NCF') <> nil then
    begin
      NcfDs := Trim(dmFactura.qryVentaFactura.FieldByName('NUMERO_NCF').AsString);
      if not SameText(NcfDs, NcfRef) then
        raise Exception.CreateFmt(
          'Inconsistencia crítica en NCR.' + sLineBreak +
          'Se solicitó el e-NCF [%s], pero la consulta quedó posicionada en [%s].' + sLineBreak +
          'No se generará la nota de crédito.',
          [NcfRef, NcfDs]);
    end;
  end;
end;

function TfrmConsultaFacturas2Ecf.GetNcrNcfRefFilter: string;
begin
  Result := Trim(edtNcrNcfRef.Text);
end;

procedure TfrmConsultaFacturas2Ecf.ApplyNcrNcfRefFilter(var ASql: string);
var
  NcfRef: string;
  P: Integer;
  OrderByPart: string;
...
begin
  ...
  SqlLower := LowerCase(ASql);

  // Evita agregar dos veces el filtro si el usuario pulsa buscar varias veces.
  if Pos('ncr_ncf_ref', SqlLower) > 0 then
    Exit;

  P := Pos(' order by ', SqlLower);
  OrderByPart := '';

  if P > 0 then
  begin
    OrderByPart := Copy(ASql, P, MaxInt);
    ASql := Copy(ASql, 1, P - 1);
  end;

  // No dependemos del alias usado por los SQL base.
  // Todas las consultas de este formulario usan VENTAS_MAST; filtramos por la serie NCF asignada.
  ASql := ASql +
    ' and exists (' +
    ' select 1 from NCF_ASIGNADOS NCF_FILTER ' +
    ' where NCF_FILTER.SERIE = VENTAS_MAST.SERIE_NCF_ASIGNADO ' +
    ' and NCF_FILTER.NUMERO_NCF = :NCR_NCF_REF' +
    ' ) ' + OrderByPart;
end;

procedure TfrmConsultaFacturas2Ecf.SetParamIfExists(
  ADataSet: TIBQuery; const AParamName, AValue: string);
begin
  if (ADataSet <> nil) and (ADataSet.Params.FindParam(AParamName) <> nil) then
    ADataSet.ParamByName(AParamName).AsString := AValue;
end;
//end Idesi2ECF
procedure TfrmConsultaFacturas2Ecf.BitBtn1Click(Sender: TObject);
var
...
begin
  ...
  // En selección NCR o cuando se filtra por e-NCF, NO mover el cursor.
  // Mover a Last puede posicionar otra factura y generar una NCR contra un NCF distinto.
  if (FPickerMode <> fpmNcrSelection) and (Trim(GetNcrNcfRefFilter) = '') then
...
procedure TfrmConsultaFacturas2Ecf.FormCreate(Sender: TObject);
begin
  FPickerMode := fpmNormal;
  FSelectedNumeroVenta := 0;
  FSelectedNumeroFactura := 0;
  FSelectedNcf := '';
  FPickerMode := fpmNormal;
  FSelectedNumeroVenta := 0;
  FSelectedNumeroFactura := 0;
  ...
  //URepRecibo8x5_5
procedure TfrmConsultaFacturas2Ecf.cboxClientesChange(Sender: TObject);
begin
  if qryClientes.Locate('NOMBRE_FACTURAR', cboxClientes.Text,[]) then
  edtCodCte.SetInteger(qryClientesCODIGO_CTE.Value);
end;

procedure TfrmConsultaFacturas2Ecf.DBEdit1Change(Sender: TObject);
begin
  if (frmConsultaFacturas2Ecf = nil)  then exit;
...
procedure TfrmConsultaFacturas2Ecf.RxDBGrid1CellClick(Column: TColumn);
begin
  DBEdit1Change(Self);
end;

procedure TfrmConsultaFacturas2Ecf.RxDBGrid1Enter(Sender: TObject);
begin
  DBEdit1Change(Self);
  UpdateEcfUi;
end;

procedure TfrmConsultaFacturas2Ecf.RxDBGrid1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  DBEdit1Change(Self);
end;

procedure TfrmConsultaFacturas2Ecf.BitBtn4Click(Sender: TObject);
begin
  if (dmFactura.qryVentaFacturaSTATUS.Value[1] in ['C','R']) then
  begin
    MessageDlg('Factura no est� activa, verifique.',mtError,[mbok],0);
    Exit;
  end;
  
  if (Not dmFactura.qryVentaFacturaFORMA_PAGO.Value In [7,8] ) then
  begin
    MessageDlg('Solo aplica para facturas a cr�dito, verifique.', mtError, [mbOk], 0);
    Exit;
  end;
    ...
    NUMERO_FACTURA = :OLD_NUMERO_FACTURA
      ...
      begin
        ...
        begin
          ...
          MessageDlg('Cliente no tiene cr�dito...', mtWarning, [mbOK], 0)
          else
          MessageDlg('Balance CXC excede limite de cr�dito.', mtWarning, [mbOK], 0);
          Exit;
        end;
...
procedure TfrmConsultaFacturas2Ecf.FormClose(Sender: TObject;
...
procedure TfrmConsultaFacturas2Ecf.BitBtn5Click(Sender: TObject);
begin
  if (dmFactura.qryVentaFacturaSTATUS.Value[1] in ['C','R']) then
  begin
    MessageDlg('Factura no est� activa, no puede ser modificada.',mtInformation,[mbok],0);
    exit;
  end;
  ...
  begin
    ...
    begin
      MessageDlg('Esta factura, tiene movimientos en CXC, no ser� cancelada, verifique.',mtWarning,[mbok],0);
      exit;
    end else
    begin
...
procedure TfrmConsultaFacturas2Ecf.RxDBGrid1GetCellProps(Sender: TObject;
...
procedure TfrmConsultaFacturas2Ecf.RxDBGrid2GetCellProps(Sender: TObject;
  Field: TField; AFont: TFont; var Background: TColor);
begin
...
procedure TfrmConsultaFacturas2Ecf.BitBtn9Click(Sender: TObject);
begin
  dmFactura.qryVentaFactura.First;
  UpdateEcfUi;
end;

procedure TfrmConsultaFacturas2Ecf.BitBtn10Click(Sender: TObject);
begin
  if dmFactura.qryVentaFactura.State = dsBrowse then
...
procedure TfrmConsultaFacturas2Ecf.BitBtn11Click(Sender: TObject);
begin
  dmFactura.qryVentaFactura.Next;
  UpdateEcfUi;
end;

procedure TfrmConsultaFacturas2Ecf.BitBtn12Click(Sender: TObject);
begin
  if dmFactura.qryVentaFactura.State = dsBrowse then
  dmFactura.qryVentaFactura.Last;
  UpdateEcfUi;
...
function TfrmConsultaFacturas2Ecf.ExecuteForNcrSelection: Boolean;
begin
  FPickerMode := fpmNcrSelection;
  ApplyPickerMode;
  Result := (ShowModal = mrOk);
end;

procedure TfrmConsultaFacturas2Ecf.ApplyPickerMode;
begin
  if FPickerMode = fpmNcrSelection then
  begin
    Caption := 'Facturas - Seleccionar para NCR';
    btnCrearNCR.Caption := 'Seleccionar factura NCR';
    btnCrearNCR.Visible := True;
    btnCrearNCR.Enabled := True;
  end
  else
  begin
    Caption := 'Facturas';
    btnCrearNCR.Caption := 'Crear NCR';
  end;
end;

procedure TfrmConsultaFacturas2Ecf.AcceptCurrentFacturaForNcr;
begin
  if dmFactura.qryVentaFactura.IsEmpty then
    raise Exception.Create('Debe seleccionar una factura.');

  if Trim(dmFactura.qryVentaFactura.FieldByName('NUMERO_NCF').AsString) = '' then
    raise Exception.Create('La factura seleccionada no tiene NCF/e-NCF asignado.');

  FSelectedNumeroVenta := dmFactura.qryVentaFactura.FieldByName('NUMERO').AsInteger;
  FSelectedNumeroFactura := dmFactura.qryVentaFactura.FieldByName('NUMERO_FACTURA').AsInteger;
  FSelectedNcf := Trim(dmFactura.qryVentaFactura.FieldByName('NUMERO_NCF').AsString);

  if (Trim(GetNcrNcfRefFilter) <> '') and
     (not SameText(FSelectedNcf, Trim(GetNcrNcfRefFilter))) then
    raise Exception.CreateFmt(
      'Inconsistencia crítica en selección NCR.' + sLineBreak +
      'El e-NCF esperado es [%s], pero la fila seleccionada tiene [%s].',
      [Trim(GetNcrNcfRefFilter), FSelectedNcf]);

  ModalResult := mrOk;
end;

procedure TfrmConsultaFacturas2Ecf.btnCrearNCRClick(Sender: TObject);
begin
  if FPickerMode = fpmNcrSelection then
  begin
    AcceptCurrentFacturaForNcr;
    Exit;
  end;
...
procedure TfrmConsultaFacturas2Ecf.BitBtn6Click(Sender: TObject);
begin
  DateTimePicker1.Date := DateTimePicker1.Date + 1;
  BitBtn1Click(Self);
end;

procedure TfrmConsultaFacturas2Ecf.BitBtn7Click(Sender: TObject);
begin
  DateTimePicker1.Date := DateTimePicker1.Date - 1;
    BitBtn1Click(Self);
end;

procedure TfrmConsultaFacturas2Ecf.btbtnEditaFactVocadoClick(Sender: TObject);
begin
...
procedure TfrmConsultaFacturas2Ecf.CheckBox2Click(Sender: TObject);
begin
...
procedure TfrmConsultaFacturas2Ecf.VerificaVendedor;
begin
...
procedure TfrmConsultaFacturas2Ecf.BitBtn8Click(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      MessageDlg('Veh�culo/Motor ya esta asignado en otra venta, verifique', mtinformation,[mbok],0);
      exit;
    end;
...
procedure TfrmConsultaFacturas2Ecf.FormShow(Sender: TObject);
begin
  ApplyPickerMode;
  if GlbMutur = 1 then
  BitBtn8.Visible:=True
  else BitBtn8.Visible:=False;
  BitBtn16.Visible:=BitBtn8.Visible;
  CartadeRuta1.Visible:=BitBtn8.Visible;
end;

procedure TfrmConsultaFacturas2Ecf.BitBtn13Click(Sender: TObject);
var
  nF : string;
begin
  if (not VerificaClave) then
  begin
    MessageDlg('Venta no ser� eliminada, verifique.', mtWarning, [mbOk], 0);
    exit;
  end;
  qryUltFact.Close;
  qryUltFact.Open;
  ...
  begin
    ...
    begin
      MessageDlg('Existe(n) secuencia(s) de factura mayor, operaci�n no procede.', mtError,[mbOk],0);
      Exit;
    end;

    if MessageDlg('Realmente deseas eliminar la factura numero: '+
    dmFactura.qryVentaFacturaNUMERO_FACTURA.AsString+'.'
    +''#13#10'Un email de notificaci�n ser� enviado.', mtError,[mbNo, mbYes],0) = mryes then
    begin
//{ temporal
      ...
      3, //TIPO_OPE SMALLINT,--1-Orden Compra 2-Venta 3-Devolucion 4-Transferencia 5-Da�ado 6-Donacion 7-Otro
...
function TfrmConsultaFacturas2Ecf.VerificaClave: Boolean;
var
 valorE:String;
begin
...
procedure TfrmConsultaFacturas2Ecf.BitBtn14Click(Sender: TObject);
begin
  qryEmpleado.close;
  qryEmpleado.open;
...
procedure TfrmConsultaFacturas2Ecf.ReimprimirFacturaNoFiscal(numTRN: Integer);
begin
if (GlBSAM = 1) then
      begin
        dmreportes.qryViewVentasMast.Close;
...
procedure TfrmConsultaFacturas2Ecf.ProcSetPathlogoTipoServicio(tipo: smallint);
begin
...
procedure TfrmConsultaFacturas2Ecf.BitBtn15Click(Sender: TObject);
begin
...
procedure TfrmConsultaFacturas2Ecf.Consultarpago1Click(Sender: TObject);
begin
  ...
  MessageDlg('Venta es a cr�dito, verifique.',mtInformation,[mbOk],0);
end;

procedure TfrmConsultaFacturas2Ecf.EditarMontoPagado1Click(Sender: TObject);
begin
  if dmFactura.qryVentaFacturaNumero.IsNull then exit;
...
procedure TfrmConsultaFacturas2Ecf.ProcInsertarDetEfectivo(numtrn,
...
procedure TfrmConsultaFacturas2Ecf.AutoAutualizaFPago1Click(Sender: TObject);
begin
  AutoAutualizaFPago1.Checked:=not AutoAutualizaFPago1.Checked;
end;

procedure TfrmConsultaFacturas2Ecf.DgiiSolicitareCF(numerotrn: integer);
...
begin
  ...
  if UUtilecf.EjecutarECF_y_Mostrar(GlbRutaEcf, IntToStr(numerotrn),_smg) then
    MessageBox(0, Pchar(_smg), 'Facturaci�n Electr�nica', MB_ICONINFORMATION or MB_OK)
  else
  MessageBox(0, Pchar(_smg), 'Facturaci�n Electr�nica', MB_ICONINFORMATION or MB_OK);
end;

procedure TfrmConsultaFacturas2Ecf.Button1Click(Sender: TObject);
begin
  if GlbActivaECF = 0 then
  begin
...
procedure TfrmConsultaFacturas2Ecf.SolicitareCFDGII1Click(Sender: TObject);
begin
  if GlbActivaECF = 0 then
  begin
    MessageDlg('No estas en modo eCF Activo',mtInformation,[mbOk],0);
...
procedure TfrmConsultaFacturas2Ecf.ieneECF1Click(Sender: TObject);
begin
  if GlbActivaECF = 0 then
  begin
    MessageDlg('No estas en modo eCF Activo',mtInformation,[mbOk],0);
...
procedure TfrmConsultaFacturas2Ecf.ConsultaTimbreeCF1Click(Sender: TObject);
begin
...
procedure TfrmConsultaFacturas2Ecf.AsignareCFSerieE1Click(Sender: TObject);
begin
...
procedure TfrmConsultaFacturas2Ecf.DgiiReenviarECF(ANumeroTrn: Integer; const AEncf: string);
var
  LMsg: string;
begin
  LMsg := '';
  ...
  if Trim(AEncf) = '' then
  begin
    MessageBox(0, 'No hay e-CF asignado a esta factura para reenviar.', 'Facturaci�n Electr�nica',
      MB_ICONWARNING or MB_OK);
    Exit;
  end;

  if not UUtilecf.EjecutarECF_Reenviar(GlbRutaEcf, IntToStr(ANumeroTrn), Trim(AEncf), LMsg) then
    MessageBox(0, PChar(LMsg), 'Reenv�o e-CF', MB_ICONERROR or MB_OK)
  else
    MessageBox(0, PChar(LMsg), 'Reenv�o e-CF', MB_ICONINFORMATION or MB_OK);
  //dmFactura.dsqryVentaFactura.DataSet.Refresh;
  UpdateEcfUi;
end;

procedure TfrmConsultaFacturas2Ecf.DgiiRefrescarCertificado(ANumeroTrn: Integer);
var
  LMsg: string;
begin
  LMsg := '';
...
procedure TfrmConsultaFacturas2Ecf.DgiiVerificarCertificado;
var
  LMsg: string;
begin
  LMsg := '';
  // El TRN no se usa para nada cr�tico en --check-cert-only, se puede mandar '0'
...
procedure TfrmConsultaFacturas2Ecf.RefrescarCert1Click(Sender: TObject);
begin
  DgiiRefrescarCertificado(dmFactura.qryVentaFacturanumero.Value);
end;

procedure TfrmConsultaFacturas2Ecf.ChequearCert1Click(Sender: TObject);
begin
  DgiiVerificarCertificado;
end;

procedure TfrmConsultaFacturas2Ecf.Reenviar1Click(Sender: TObject);
begin
  DgiiReenviarECF(dmFactura.qryVentaFacturanumero.Value, dmFactura.qryVentaFacturaNUMERO_NCF.Value);
end;

function TfrmConsultaFacturas2Ecf.GetCurrentEcfEstado: string;
begin
  if (dmFactura.dsqryVentaFactura.DataSet = nil) or dmFactura.dsqryVentaFactura.DataSet.IsEmpty then
    Result := ''
  else
    // AJUSTA ESTE CAMPO: aqu� va el estado que guarda IdesiToolECF
...
procedure TfrmConsultaFacturas2Ecf.UpdateEcfUi;
var
  hasRow   : Boolean;
  trn      : Integer;
  encf     : string;
...
begin
  ...
  // Estados que consideramos "candidatos a env�o" (sin aceptar / sin proceso activo)
  canSend :=
    (estado = '') or
    (estado = 'PENDIENTE') or
    (estado = 'RECHAZADO') or
  ...
  // Reenviar: solo si hay ENCF y NO est� aceptado ni "En proceso"
  canResend :=
    (encf <> '') and
    (estado <> 'ACEPTADO') and
    (estado <> 'IN_PROCESS');

  // RefreshCert: igual que enviar, pero no exigimos que encf est� vac�o
  canRefresh :=
    (estado <> 'ACEPTADO') and
    (estado <> 'IN_PROCESS');
  ...
  // Reglas finales
  //SolicitareCFDGII1.Enabled:= canSend   and (not GetVerificaeCF(trn));  // Enviar normal solo si a�n no tiene ENCF
  Reenviar1.Enabled     := canResend;
  RefrescarCert1.Enabled:= canRefresh;

  // Sincronizar botones con men�s
  {btnECFEnviar.Enabled      := mnuECFEnviar.Enabled;
  btnECFReenviar.Enabled    := mnuECFReenviar.Enabled;
  btnECFRefreshCert.Enabled := mnuECFRefreshCert.Enabled;}
end;

procedure TfrmConsultaFacturas2Ecf.CambiarTipoECF1Click(Sender: TObject);
var
  Modo: Integer;
  RNC, Nombre: string;
  Trn: Integer;
...
begin
  ...
  try
    ...
    begin
      // usuario acept� ? Modo, RNC y Nombre vienen actualizados
      xflag := False;
    end else
    xflag := True;
  finally
  ...
  if xflag then Exit;
    ...
    // Aqu� usas "Modo", "RNC" y "Nombre" para ejecutar el flujo real:
    if Modo = 0 then
    begin
      ...
      begin
        MessageDlg('Transacci�n no existe.',mtinformation,[mbok],0);
        exit;
      end;
...
procedure TfrmConsultaFacturas2Ecf.Convertir_Modo0_CambiarE32porE31(ds: TIBDataSet;codcte:integer);
var
  oldTrn: Integer;
...
begin
  ...
  // Capturar RNC/NOMBRE del comprador para convertir a cr�dito fiscal
  if (rnc = '') then
  begin
    if not CapturarRNCComprador(rnc, nombre,codcte) then
      Exit;
  ...
  if MessageDlg(Format(
      'Se va a convertir la factura de consumo E32 (TRN %d) en una factura de cr�dito fiscal E31.'#13#10 +
      'No se crear� nueva venta ni nota de cr�dito.'#13#10#13#10 +
...
procedure TfrmConsultaFacturas2Ecf.Convertir_Modo1_NuevaE31_mas_NC(ds: TIBDataSet;codcte:integer;
    oldTrn: Integer;tipoEcf: string;rnc, nombre: string);
var
...
begin
  ...
  if MessageDlg(Format(
      'Se crear� una NUEVA factura de Cr�dito Fiscal (E31) copiando los �tems de la factura de consumo E32 (TRN %d).'#13#10 +
      'Luego se generar� autom�ticamente una Nota de Cr�dito (E34) que anular� la E32.'#13#10#13#10 +
      '�Desea continuar?', [oldTrn]),
      mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
    Exit;

  // Aqu� delegamos la l�gica de clonaci�n/NC a helpers (que a su vez
  ...
  if ncTrnE34 = 0 then
    raise Exception.Create('No se pudo crear la Nota de Cr�dito E34 que anula la E32.')
  else
  begin
    ds.Edit;
  ...
  ShowMessage(Format(
    'Conversi�n completada (Modo 1):'#13#10 +
    '- Nueva factura E31 TRN %d'#13#10 +
    '- Nota de Cr�dito E34 TRN %d (anula la E32 TRN %d)',
    [newTrnE31, ncTrnE34, oldTrn]));
end;


function TfrmConsultaFacturas2Ecf.CapturarRNCComprador(var ARNC, ANombre: string; var codcte:integer): Boolean;
begin
  ...
  //ANombre := InputBox('Nombre / Raz�n Social', 'Digite el nombre del comprador:', '');
  //Result := Trim(ANombre) <> '';
...
function TfrmConsultaFacturas2Ecf.CrearVentaCreditoDesdeConsumo(ATrnConsumo: Integer;
...
begin
  ...
  //    Aqu� usas tu l�gica existente: buscar por RNC, o crear si no existe.
  ...
  try
    ...
    q.ParamByName('A_TIPO_DOC_PAGO').AsInteger := 1;            // c�digo que uses para E31
...
function TfrmConsultaFacturas2Ecf.CrearNotaCreditoAnulacion(
...
procedure TfrmConsultaFacturas2Ecf.eNCFAsignados1Click(Sender: TObject);
begin
...
procedure TfrmConsultaFacturas2Ecf.SpeedButton1Click(Sender: TObject);
begin
  FiltrarSoloMonto:=True;
  BitBtn1Click(Self);
end;

procedure TfrmConsultaFacturas2Ecf.BitBtn16Click(Sender: TObject);
begin
...
//UQckRepCartaRutaVeh
procedure TfrmConsultaFacturas2Ecf.CartadeSaldo1Click(Sender: TObject);
begin
  if dmDatos.qryMembrete.state = dsInactive then
  dmDatos.qryMembrete.Open;
