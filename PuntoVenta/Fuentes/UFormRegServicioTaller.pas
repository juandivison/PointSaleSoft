...
type
  TfrmRegServicioTaller = class(TForm)
    ...
    SkinData1: TSkinData;
    ibstpProc_Ins_ServTExtraDet: TIBStoredProc;
  ...
  private
    ...
    Procedure AbrirDatosRepFEntrega;
    Procedure ProcInsPosExtraDet(numstr:integer);
  public
    { Public declarations }
    xCerrar : boolean;
  end;
...
uses UDatModConectar, UDatosVentas, UDatModPrestamo, UDatmodDatosGenerales, UGlobal,
...
begin
  ...
  begin
    ...
    //temporal frmProcVentaRapida.DBEdit1Exit(Self);
...
begin
  ...
  if Assigned(frmProcVentaRapida) then
  begin
    frmProcVentaRapida.qryCliente.Close;
    frmProcVentaRapida.qryCliente.Open;
  end;
end;
...
begin
  ...
  begin
    canClose:=False;
    if MessageDlg('Hay una transacción en curso. Aún desea salir?',mtInformation,[mbyes,mbno],0) = mrYes then
    canClose:=True;
  end else
  begin
    canClose:=True;
...
procedure TfrmRegServicioTaller.ReportexFechaEntrega1Click(
  Sender: TObject);
begin
  AbrirDatosRepFEntrega;
  if dmReportes.qryRepServTallerM.RecordCount > 0 then
  begin
    qckListaServtXVendedor:=TqckListaServtXVendedor.Create(Nil);
    try
      qckListaServtXVendedor.QRLabel6.Caption:='F. Entrega';
      qckListaServtXVendedor.QRDBText2.DataField:='fecha_estimada_entrega';
      qckListaServtXVendedor.QRDBText2.Mask := 'dd/mm/yy';
      qckListaServtXVendedor.qrLabelCriterioDatos.Caption:= labelCretrioRep;
      if MessageDlg('Imprimir?', mtinformation,[mbyes,mbno],0) = mrok then
      begin
        qckListaServtXVendedor.PrinterSetup;
        qckListaServtXVendedor.Print;
      end else
      qckListaServtXVendedor.Preview;
    finally
    qckListaServtXVendedor.Free;
...
procedure TfrmRegServicioTaller.AbrirDatosRepFEntrega;
begin
  GetAnyDate1.FechaCierre := ExtraerFecha(GlbFechaTrnDiaria);
  if GetAnyDate1.Execute then
  begin
  ...
  dmReportes.qryRepServTallerM_base.SQL.Text + format(' Where SERVICIOS_MASTER.Status in (%s,%s) and  SERVICIOS_MASTER.fecha_estimada_entrega Between %s and %s',
  [chr(39)+'A'+chr(39),chr(39)+'R'+chr(39),chr(39)+FormatDateTime('mm/dd/yyyy',GetAnyDate1.Fecha)+chr(39),
  chr(39)+FormatDateTime('mm/dd/yyyy',  GetAnyDate1.FechaFinal)+chr(39)])+
  format(' and SERVICIOS_MASTER.CODIGO_EMP Between %d and  %d ',[0, 1000])+
  ' order by SERVICIOS_MASTER.CODIGO_EMP ';
  labelCretrioRep:= 
  FormatDateTime('dd/mm/yyyy',GetAnyDate1.fecha)
  + ' ' +' Al '+FormatDateTime('dd/mm/yyyy',GetAnyDate1.fechaFinal);

  dmReportes.qryRepServTallerDet.Close;
  dmReportes.qryRepServTallerDet.Params[0].Value:= ExtraerFecha(GetAnyDate1.fecha);
  dmReportes.qryRepServTallerDet.Params[1].Value:= ExtraerFecha(GetAnyDate1.fechaFinal);
  dmReportes.qryRepServTallerM.SQL.Text;
  dmReportes.qryRepServTallerM.Open;
  end;
end;

procedure TfrmRegServicioTaller.BuscarCliente1Click(Sender: TObject);
begin
  FrmBuscarClientesPersonas:=TFrmBuscarClientesPersonas.Create(nil);
  try
    FrmBuscarClientesPersonas.CodigoCliente := -1;
    if FrmBuscarClientesPersonas.Showmodal = mrOk then
    begin
      if dmVentas.tblRegServicioMaster.Locate('CODIGO_CTE',FrmBuscarClientesPersonas.CodigoCliente,[]) then
      MessageDlg('No encontrado.',mtInformation,[mbok],0);
    end;
  finally
  FrmBuscarClientesPersonas.Free;
  FrmBuscarClientesPersonas:=Nil;
...
procedure TfrmRegServicioTaller.ProcInsPosExtraDet(numstr: integer);
begin
{
  stored procedure: PROC_INS_ServTEXTRADET
  ibstpProc_Ins_ServTExtraDet.Params[0].Value:=  
  ibstpProc_Ins_ServTExtraDet.Params[1].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[2].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[3].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[4].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[5].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[6].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[7].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[8].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[9].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[10].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[11].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[12].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[13].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[14].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[15].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[16].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[17].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[18].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[19].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[20].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[21].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[22].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[23].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[24].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[25].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[26].Value:=
  ibstpProc_Ins_ServTExtraDet.Params[27].Value:=}
end;
