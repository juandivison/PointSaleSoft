...
type
  TfrmContOrdenReg = class(TfrmContabilizaDatos)
    Memo1: TMemo;
    DBGridTest: TDBGrid;
    ...
    qryviewDatosParaCntTIPO_TRN: TSmallintField;
    qryviewDatosParaCntFECHA: TDateTimeField;
    qryviewDatosParaCntMONTO_TOTAL: TFloatField;
    qryviewDatosParaCntFORMA_PAGO: TSmallintField;
    qryviewDatosParaCntCODIGO_PROD: TIntegerField;
    ...
    qryviewDatosParaCntVALOR_BRUTO: TFloatField;
    qryviewDatosParaCntVALOR_TOTAL_DET: TFloatField;
    qryviewDatosParaCntNUM_ORDEN: TFloatField;
    qryviewDatosParaCntSTATUS_DET: TIBStringField;
    qryviewDatosParaCntDESCRIPCION: TIBStringField;
    qryviewDatosParaCntCANT_UNIDADES: TFloatField;
    qryviewDatosParaCntSTATUS_CNT: TIBStringField;
...
var
  frmContOrdenReg: TfrmContOrdenReg;
...
procedure TfrmContOrdenReg.FormCreate(Sender: TObject);
var 
 lfecha: TDatetime;
 lstatus: String;
begin
...
procedure TfrmContOrdenReg.btnDatosClick(Sender: TObject);
var
  lfecha :TDatetime;
  lstatus:String;
  nError:Boolean;
...
procedure TfrmContOrdenReg.PrepararDatos;
var
  campos : String;
  strcampokey : string;
  x, y : Integer;
...
procedure TfrmContOrdenReg.ProcDatos;
var
  xmonto  : Extended;
  fCampos : TField;
  NombreCampo : string;
begin
  GlbDescTipoCNT := 'Transacccion Orden Compra';
...
procedure TfrmContOrdenReg.HacerTotales;
begin
...
procedure TfrmContOrdenReg.btnProcesarClick(Sender: TObject);
var
  tempo : extended;
begin
  //inherited;
  ...
  dmCxc.rxTranCnt.Close;
  dmCxc.rxTranCnt.Open;
  GlbDescTipoCNT := 'Transaccción Orden Compra';
  ...
  frmPrepararDatosCnt.FTipoTRN      := FPago; //Venta al Contado
  frmPrepararDatosCnt.esVentaCash:= False;
  frmPrepararDatosCnt.nombreView := 'view_RegPedidoACnt';
  frmPrepararDatosCnt.esTarjeta := False;
  frmPrepararDatosCnt.ProcDatosOrdenes(dmCxc.rxTranCnt);
...
procedure TfrmContOrdenReg.btnContabilizarClick(Sender: TObject);
begin
  inherited;
  //
  btnContabilizar.Enabled:=False;
...
function TfrmContOrdenReg.procPreparaCampootro(xnum : Integer):Boolean;
var
  campos : string;
begin
  Memo1.Clear;
...
procedure TfrmContOrdenReg.dtpkfechaExit(Sender: TObject);
begin
  inherited;
  //
end;
