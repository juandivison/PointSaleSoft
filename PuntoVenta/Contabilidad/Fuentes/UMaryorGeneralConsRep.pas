...
uses
  ...
  DBGrids, Db, DBTables, pr_Dataset, variants, WinSkinData;
...
type
  TfrmMayorGeneral = class(TForm)
    ...
    BitBtn4: TBitBtn;
    prEventosRepMayorGen: TprEventsDataset;
    SkinData1: TSkinData;
...
Uses UDmRep, UGlobal, URepMayorGeneral,
  URepMayorDetallado, UFormaRepParaDos, UDatModControl;
...
begin
  ...
  dtmodrep.tClase_cuentas_adm.Close;
  dtmodrep.tClase_cuentas_adm.Open;
...
begin
  ...
  //dmControl.qrycontrol.Filter:=' sistema         = 1 '+
  //                      ' and sub_sistema = 0 '+
  //                      ' and cod_cia     = 2 ';
  ...
  begin
   MessageDlg('Modulo transacciones no encontrado, verifique.', mtInformation, [mbOK], 0);
  end;
  //dmControl.qrycontrol.Filtered:=true; //Transacciones facturas
  DecodeDAte(dmControl.qrycontrolfecha_act.value,tanio,tmes,tdia);
...
end.procedure TfrmMayorGeneral.prEventosRepMayorGenActive(
  prDataset: TprDataset; var IsActive: Boolean);
begin
